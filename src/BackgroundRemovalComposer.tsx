...
import { buildGptBackgroundRemovalPrompt, chooseContrastingBackground, hasUsableTransparency, removeImageBackground } from './services/backgroundRemoval';
...
interface Item { id: string; file: File; sourceUrl: string; status: 'waiting' | 'running' | 'success' | 'failed'; progress?: number; stage?: string; matteColor?: string; aiResultBlob?: Blob; aiResultUrl?: string; resultBlob?: Blob; resultUrl?: string; vectorBlob?: Blob; vectorStatus?: 'converting' | 'ready' | 'failed'; error?: string }
const CONCURRENCY = 2;
...
export default function BackgroundRemovalComposer({ openAiApiKey, onRequestKey, onSessionStateChange, settingsHost }: {
  ...
  const processItem = async (item: Item) => {
    ...
    try {
      ...
      const resultBlob = await hasUsableTransparency(intermediate)
        ? intermediate
        : await removeImageBackground(intermediate, ({ percent }) => patchItem(item.id, { progress: percent }), settings);
  ...
  const run = async (targets = items) => {
    ...
    if (settings.mode === 'gpt-hybrid' && !openAiApiKey) return onRequestKey();
  ...
  const panel = <div className="settings-panel"><Title level={4}>智能抠图</Title><Form layout="vertical"><Form.Item label="处理方式"><Select value={settings.mode} onChange={(mode) => patchSettings({ mode })} options={[{ value: 'local', label: '本地智能抠图（快速）' }, { value: 'gpt-hybrid', label: 'GPT 复杂图精细抠图' }]} /></Form.Item>{settings.mode === 'gpt-hybrid' && <><Form.Item label="GPT 输出质量"><Select value={settings.quality} onChange={(quality) => patchSettings({ quality })} options={['high', 'medium', 'low'].map((value) => ({ value, label: value }))} /></Form.Item><Form.Item label="GPT 分离底色" tooltip="自动模式会选择与原图主体最不相似的高反差颜色"><Select value={settings.gptBackground} onChange={(gptBackground) => patchSettings({ gptBackground })} options={[{ value: 'auto', label: '自动避开主体颜色（推荐）' }, { value: '#FF00FF', label: '🟪 品红 #FF00FF' }, { value: '#00FFFF', label: '🟦 青色 #00FFFF' }, { value: '#00FF00', label: '🟩 绿色 #00FF00' }, { value: '#FFFF00', label: '🟨 黄色 #FFFF00' }, { value: '#000000', label: '⬛ 黑色 #000000' }, { value: '#FFFFFF', label: '⬜ 白色 #FFFFFF' }]} /></Form.Item></>}<Form.Item label={`主体保留：扩展 ${settings.edgeExpansion}px`} tooltip="主体被抠掉太多时调大；背景残留时调小"><Slider min={0} max={8} step={1} value={settings.edgeExpansion} onChange={(edgeExpansion) => patchSettings({ edgeExpansion })} marks={{ 0: '精确', 2: '推荐', 8: '保留更多' }} /></Form.Item><Form.Item label={`边缘柔化：${settings.edgeFeather}px`} tooltip="减少锯齿；过大会让边缘发虚"><Slider min={0} max={4} step={1} value={settings.edgeFeather} onChange={(edgeFeather) => patchSettings({ edgeFeather })} marks={{ 0: '清晰', 1: '推荐', 4: '柔和' }} /></Form.Item></Form><Text type="secondary">{settings.mode === 'local' ? '浏览器本地处理，不需要 API Key。主体被误删时提高“主体保留”。' : 'GPT 先生成高反差纯色底，本地模型再生成透明 Alpha；可通过 AI 返回图检查底色效果。'}</Text></div>;
  ...
  return <div className="background-removal-page"><section className="hero-strip background-removal-hero"><div><Text className="eyebrow">BACKGROUND REMOVER</Text><Title level={2}>一键智能抠图，直接生成透明背景</Title><Paragraph className="hero-description">专用分割模型直接识别主体与精细边缘，不使用提示词、不重绘图片，也不再通过绿色底色转换透明。</Paragraph></div><div className="hero-orb" /></section>
    ...
    <section className="results-section">
      ...
      {items.length ? <Image.PreviewGroup><div className="logo-replace-results">{items.map((item) => <Card key={item.id} size="small" title={item.file.name} extra={<Space>{item.aiResultBlob && <Button type="text" icon={<DownloadOutlined />} onClick={() => downloadBlob(item.aiResultBlob!, `${sanitizeFileName(item.file.name)}_AI返回.png`)}>AI 原图</Button>}{item.resultBlob && <Button type="text" icon={<DownloadOutlined />} onClick={() => downloadBlob(item.resultBlob!, `${sanitizeFileName(item.file.name)}_透明.png`)} />}</Space>}>
        ...
        <Flex justify="space-between" align="center" style={{ marginTop: 8 }}><Space size={4}><Tag color={item.status === 'success' ? 'success' : item.status === 'failed' ? 'error' : item.status === 'running' ? 'processing' : 'default'}>{item.status}</Tag>{item.matteColor && <Tag><span style={{ display: 'inline-block', width: 10, height: 10, marginRight: 5, borderRadius: 2, verticalAlign: -1, background: item.matteColor, border: '1px solid #999' }} />底色 {item.matteColor}</Tag>}</Space><Space><Button size="small" icon={<ReloadOutlined />} disabled={busy} onClick={() => void run([item])}>按当前参数重抠</Button>{item.resultBlob && <Button size="small" type="primary" ghost icon={<ThunderboltOutlined />} loading={item.vectorStatus === 'converting'} onClick={() => item.vectorBlob ? downloadBlob(item.vectorBlob, `${sanitizeFileName(item.file.name)}_矢量.svg`) : void vectorize(item)}>{item.vectorBlob ? '下载 SVG' : '转为矢量图'}</Button>}</Space></Flex>
