...
import { restoreTransparentBackground } from './services/transparentImageEdit';
import { createId, downloadBlob, sanitizeFileName } from './utils';

const { Text, Title, Paragraph } = Typography;
const ACCEPTED = ['image/png', 'image/jpeg', 'image/webp'];
type RemovalMode = 'local' | 'gpt-hybrid' | 'gpt-color-key' | 'gpt-direct';
...
export default function BackgroundRemovalComposer({ openAiApiKey, onRequestKey, onSessionStateChange, settingsHost }: {
  ...
  const processItem = async (item: Item) => {
    patchItem(item.id, { status: 'running', progress: 0, stage: settings.mode === 'gpt-direct' ? 'GPT 正在直接生成透明 PNG' : settings.mode === 'gpt-color-key' ? 'GPT 正在生成纯色背景版本' : settings.mode === 'gpt-hybrid' ? 'GPT 正在分离复杂主体' : '本地模型正在分析主体', error: undefined, alphaStatus: undefined });
    try {
      ...
      const resultBlob = settings.mode === 'gpt-direct'
        ? intermediate
        : settings.mode === 'gpt-color-key' && selectedMatteColor
      ...
      patchItem(item.id, { status: 'success', progress: 100, resultBlob, resultUrl, resultMode: settings.mode, alphaStatus: settings.mode === 'gpt-direct' ? (hasAlpha ? 'valid' : 'missing') : settings.mode === 'gpt-color-key' ? 'valid' : undefined, vectorBlob: undefined, vectorStatus: undefined });
