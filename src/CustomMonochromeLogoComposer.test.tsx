...
import { EngravingTaskComposer as CustomMonochromeLogoComposer } from "./CustomMonochromeLogoComposer";
...
it.each([false, true])('keeps both real task results visible when concurrent responses finish in either order (reverse=%s)', async (reverse) => {
 ...
 await waitFor(()=>expect(screen.getAllByRole('region',{name:'生成结果'})).toHaveLength(2), {timeout:10000});
 expect(screen.queryByRole('heading',{name:/原照：[AB].png/})).toBeNull();
 ...
 expect(screen.getAllByRole('status').filter(node=>node.textContent==='第 1 次生成中')).toHaveLength(2);
 ...
 for(const gallery of galleries) {expect(within(gallery).getByRole('status')).toHaveTextContent('已完成');expect(gallery).toBeVisible();expect(within(gallery).getByText('第 1 张 · 未评分 · 共 1 张')).toBeVisible();}
