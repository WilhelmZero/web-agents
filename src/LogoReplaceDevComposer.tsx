...
export default function LogoReplaceDevComposer({ apiKey, apiBaseUrl, connectionMode, onRequestKey, onSessionStateChange, settingsHost, integrated = false }: {
  apiKey: string; apiBaseUrl: string | null; connectionMode: 'direct' | 'proxy'; onRequestKey: () => void;
  onSessionStateChange?: (value: boolean) => void; settingsHost?: HTMLElement | null; integrated?: boolean;
}) {
  const { message } = AntApp.useApp();
  const [settings, setSettings] = useState<LogoReplaceSettings>(() => ({ ...DEFAULT_LOGO_REPLACE_SETTINGS } as LogoReplaceSettings));
  const [multiEnabled, setMultiEnabled] = useState(integrated);
  ...
  const addLogoFile = (file: File) => { if (file.name.toLowerCase().endsWith('.psd')) { setPendingPsdFile(file); return false; } return addLogos([file]); };
  ...
  const outputName = (task: LogoReplaceDevTask) => logoReplaceResultFileName(scenes.find((scene) => scene.id === task.sceneId)?.name || `场景_${task.sceneIndex + 1}`, task.copyIndex, settings.copiesPerScene, task.resultMimeType);
