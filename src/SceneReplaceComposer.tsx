...
export default function SceneReplaceComposer({ apiKey, openAiApiKey, apiBaseUrl, connectionMode, onRequestKey, onSessionStateChange, settingsHost, initialPrompt, initialPerImagePrompts, onPerImagePromptsChange, automationStartToken, automationRetryFailedToken, automationStopToken, onProgressChange, onResultsChange }: {
  ...
  const start = async () => {
    ...
    clearResults(); setTasks(eligible.flatMap((scene) => { const sceneIndex = scenes.findIndex((item) => item.id === scene.id); const assigned = assignments[perImagePromptFileKey(scene.file)]?.prompt.trim(); const targetPrompt = config.perImagePromptEnabled ? assigned! : config.autoRecommendScene ? `${sceneThemes[scene.id]}，${prompt.trim()}` : prompt.trim(); return Array.from({ length: config.copiesPerScene }, (_, copyIndex) => ({ id: createId(), sceneId: scene.id, sceneIndex, copyIndex, status: 'waiting' as const, prompt: buildSceneReplacementPrompt(targetPrompt), retryCount: 0 })); }));
  };
