...
export default function SceneReplaceComposer({ apiKey, openAiApiKey, apiBaseUrl, connectionMode, onRequestKey, onSessionStateChange, settingsHost, initialPrompt, initialSettings, perImagePromptPrefix, exactPromptControl = false, initialPerImagePrompts, onPerImagePromptsChange, automationStartToken, automationRetryFailedToken, automationStopToken, onProgressChange, onResultsChange }: {
  ...
  const execute = useCallback(async (task: SceneReplaceTask) => {
    ...
    try {
      ...
      const resultBlob = isOpenAiModel(config.imageModel)
        ? await editPaperTextOpenAi({ apiKey: openAiApiKey, model: config.imageModel, image: scene.file, prompt: task.prompt, quality: config.imageQuality, signal: controller.signal, exactPrompt: exactPromptControl })
        : (await generateSceneReplacementImage({ apiKey, apiBaseUrl, signal: controller.signal, model: config.imageModel as ImageModel, prompt: task.prompt, image: scene.file, aspectRatio: config.ratioMode === 'fixed' ? config.aspectRatio : undefined, imageSize: config.imageSize, exactPrompt: exactPromptControl })).blob;
      let insufficientChangeWarning: string | undefined; let changedRatio: number | undefined;
  ...
  const executeBatch = async (batchTasks: SceneReplaceTask[], config: SceneReplaceSettings) => {
    ...
    try {
      ...
      for (const task of batchTasks) {
        ...
        let finalResult = result; let retryCount = task.retryCount; let insufficientChangeWarning: string | undefined; let changedRatio: number | undefined;
        if (config.detectInsufficientSceneChange) {
          const scene = scenesRef.current.find((candidate) => candidate.id === task.sceneId)!;
          while (true) {
            const change = await detectImageChange(scene.file, finalResult.blob); changedRatio = change.changedRatio;
            const detail = `场景变化检测未通过：仅 ${(change.changedRatio * 100).toFixed(1)}% 像素发生明显变化，不超过 20%`;
            ...
            const next = retried[task.id];
            if (!next || next instanceof Error) throw next || new Error('Batch 重试未返回结果');
            finalResult = next;
          }
        }
  ...
  const start = async () => {
    ...
    if (!exactPromptControl && settings.autoRecommendScene && eligible.some((scene) => !sceneThemes[scene.id])) return void message.warning('请等待所有参与生成的图片完成场景推荐，或手动填写缺少的主题');
    let assignments = perImagePrompts.assignments;
    if (!exactPromptControl && config.perImagePromptEnabled) {
  ...
  const selectedResult = useMemo(() => resultItems.find((item) => item.task.id === selectedResultId), [resultItems, selectedResultId]);
  const selectedResultGroup = useMemo(() => selectedResult ? resultItems.filter((item) => item.task.sceneId === selectedResult.task.sceneId) : [], [resultItems, selectedResult]);
  const selectedOutpaintResults = useMemo(() => selectedResultGroup.flatMap(({ task, scene }) => (task.outpaintResults || (task.outpaintBlob && task.outpaintUrl ? [{ width: settings.outpaintWidth, height: settings.outpaintHeight, blob: task.outpaintBlob, url: task.outpaintUrl }] : [])).map((result) => ({ task, scene, result }))), [selectedResultGroup, settings.outpaintHeight, settings.outpaintWidth]);
  ...
  const panel = <div className="settings-panel scene-replace-settings-panel"><Flex justify="space-between"><Title level={4} style={{ margin: 0 }}>场景替换设置</Title><Tag color="cyan">SCENE</Tag></Flex><Form layout="vertical" style={{ marginTop: 20 }}>
    ...
    <Form.Item label="上传后自动推荐场景"><Flex justify="space-between" align="center"><Text>扫描杯子、人物和构图，生成非节日场景主题</Text><Switch checked={exactPromptControl ? false : settings.autoRecommendScene} disabled={exactPromptControl} onChange={(autoRecommendScene) => { patch({ autoRecommendScene }); setPrompt((current) => { const trimmed = current.trim(); if (!trimmed || trimmed === SCENE_COMMON_CONSTRAINT || trimmed === SCENE_MANUAL_DEFAULT_PROMPT) return autoRecommendScene ? SCENE_COMMON_CONSTRAINT : SCENE_MANUAL_DEFAULT_PROMPT; return current; }); if (autoRecommendScene) void recommendThemes(scenes); }} /></Flex>{!exactPromptControl && settings.autoRecommendScene && <Space.Compact style={{ width: '100%', marginTop: 10 }}><Select value={settings.sceneRecommendationProvider} onChange={(sceneRecommendationProvider) => patch({ sceneRecommendationProvider })} options={[{ value: 'gemini', label: 'Gemini' }, { value: 'openai', label: 'GPT' }]} />{settings.sceneRecommendationProvider === 'openai' ? <Select value={settings.openAiSceneRecommendationModel} onChange={(openAiSceneRecommendationModel) => patch({ openAiSceneRecommendationModel })} options={[{ value: 'gpt-5.6-terra', label: 'GPT-5.6 Terra' }, { value: 'gpt-5.6-sol', label: 'GPT-5.6 Sol' }, { value: 'gpt-5.6-luna', label: 'GPT-5.6 Luna' }]} /> : <Select value={settings.sceneRecommendationModel} onChange={(sceneRecommendationModel) => patch({ sceneRecommendationModel })} options={[{ value: 'gemini-3.1-flash-lite', label: 'Gemini 3.1 Flash Lite' }, { value: 'gemini-3.1-flash', label: 'Gemini 3.1 Flash' }, { value: 'gemini-2.5-flash', label: 'Gemini 2.5 Flash' }]} />}</Space.Compact>}</Form.Item>
...
<Card className="workflow-card" title={<Space><span className="step-badge">2</span><span>{exactPromptControl ? '主控页完全控制提示词' : settings.autoRecommendScene ? '输入公共提示词' : '选择并编辑目标场景'}</span></Space>}><div className="scene-replace-editor">{!exactPromptControl && !settings.autoRecommendScene && <div><Flex justify="space-between" align="center"><Text strong>提示词预设</Text><Button size="small" icon={<PlusOutlined />} onClick={openPresetEditor}>新增预设</Button></Flex><div className="scene-preset-grid">{allPresets.map((preset) => <div role="button" tabIndex={0} key={preset.id} className={selectedPreset === preset.id ? 'scene-preset-card is-selected' : 'scene-preset-card'} onClick={() => { setSelectedPreset(preset.id); setPrompt(preset.content); }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { setSelectedPreset(preset.id); setPrompt(preset.content); } }}><span>{preset.icon}</span><b>{preset.name}</b>{!preset.builtIn && <Button className="scene-preset-delete" type="text" danger size="small" icon={<DeleteOutlined />} aria-label={`删除 ${preset.name}`} onClick={(event) => { event.stopPropagation(); setCustomPresets((current) => current.filter((item) => item.id !== preset.id)); if (selectedPreset === preset.id) setSelectedPreset(undefined); }} />}</div>)}</div></div>}<div className="scene-prompt-editor"><Flex justify="space-between" align="center"><Text strong>{exactPromptControl ? '来自多标签主控页的最终提示词' : settings.autoRecommendScene ? '公共约束提示词' : '发送给模型的完整提示词（可直接修改）'}</Text>{!exactPromptControl && selectedPreset && <Button type="link" size="small" onClick={() => { setSelectedPreset(undefined); setPrompt(''); }}>清除选择</Button>}</Flex><Input.TextArea value={submittedPrompt} readOnly={exactPromptControl} onChange={(event) => { setPrompt(event.target.value); setSelectedPreset(undefined); }} placeholder="例如：改为海边度假主题，严格要求杯子的外形、轮廓、比例、结构、尺寸、朝向及在图中的位置完全不变，禁止拉伸、压缩、弯曲、重塑或改变杯口、杯身、杯底；人物在图中的位置和人物动作、特别是人物手势不变，穿搭及背景氛围可改变，真实的景深效果" autoSize={{ minRows: 9, maxRows: 15 }} showCount maxLength={3000} />{exactPromptControl ? <Alert type="warning" showIcon title="提示词由多标签主控页锁定" description="生成时逐字提交上方内容，不追加模板、逐图分析、文件夹建议或系统强制限制。" /> : <><div className="scene-prompt-optimizer"><Segmented value={settings.promptOptimizerProvider} onChange={(promptOptimizerProvider) => patch({ promptOptimizerProvider: promptOptimizerProvider as SceneReplaceSettings['promptOptimizerProvider'] })} options={[{ value: 'gemini', label: 'Gemini' }, { value: 'openai', label: 'GPT' }]} />{settings.promptOptimizerProvider === 'openai' ? <Select value={settings.openAiPromptOptimizerModel} onChange={(openAiPromptOptimizerModel) => patch({ openAiPromptOptimizerModel })} options={[{ value: 'gpt-5.6-terra', label: 'GPT-5.6 Terra（推荐）' }, { value: 'gpt-5.6-sol', label: 'GPT-5.6 Sol（最高质量）' }, { value: 'gpt-5.6-luna', label: 'GPT-5.6 Luna（低成本）' }]} /> : <Select value={settings.promptOptimizerModel} onChange={(promptOptimizerModel) => patch({ promptOptimizerModel })} options={[{ value: 'gemini-3.1-flash-lite', label: 'Gemini 3.1 Flash Lite' }, { value: 'gemini-3.1-flash', label: 'Gemini 3.1 Flash' }, { value: 'gemini-2.5-flash', label: 'Gemini 2.5 Flash' }]} />}<Button type="primary" icon={<BulbOutlined />} loading={optimizingPrompt} onClick={() => void optimizeScenePrompt()}>优化提示词</Button></div><Alert type="info" showIcon title="完整提示词完全由你控制" description="优化会增强摄影和场景描述，同时保留原提示词中的杯子、人物位置、动作和手势等硬性约束；优化结果仍可继续修改。" /><Button icon={<PlusOutlined />} onClick={openPresetEditor}>将当前提示词保存为预设</Button></>}</div></div></Card>
    {!exactPromptControl && settings.perImagePromptEnabled && <Card className="workflow-card" title="逐图提示词分配" extra={<Button icon={<ReloadOutlined />} disabled={!promptReviewScenes.length} onClick={() => void perImagePrompts.analyze()}>分析全部 / 重试失败</Button>}>{settings.autoSkipWhiteBackground && whiteBackgroundSceneIds.length > 0 && <Alert type="info" showIcon style={{ marginBottom: 12 }} title={`${whiteBackgroundSceneIds.length} 张白底图无需生成，已从逐图提示词审核中排除`} />}<div className="per-image-prompt-grid">{promptReviewScenes.map((scene) => <Card size="small" key={scene.id} title={scene.name}><PerImagePromptEditor file={scene.file} assignment={perImagePrompts.assignments[perImagePromptFileKey(scene.file)]} sourcePrompt={perImagePromptSource} onEdit={(value) => perImagePrompts.edit(scene.file, value)} onAnalyze={() => void perImagePrompts.analyze([scene.file])} /></Card>)}</div></Card>}
