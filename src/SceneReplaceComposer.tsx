...
export default function SceneReplaceComposer({ apiKey, openAiApiKey, apiBaseUrl, connectionMode, onRequestKey, onSessionStateChange, settingsHost, initialPrompt, automationStartToken, automationRetryFailedToken, automationStopToken, onProgressChange, onResultsChange }: {
  ...
  const start = () => {
    ...
    clearResults(); setTasks(eligible.flatMap((scene) => { const sceneIndex = scenes.findIndex((item) => item.id === scene.id); return Array.from({ length: settings.copiesPerScene }, (_, copyIndex) => ({ id: createId(), sceneId: scene.id, sceneIndex, copyIndex, status: 'waiting' as const, prompt: settings.autoRecommendScene ? `${sceneThemes[scene.id]}，${prompt.trim()}` : prompt.trim(), retryCount: 0 })); }));
  };
