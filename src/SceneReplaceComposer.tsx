...
export default function SceneReplaceComposer({ apiKey, openAiApiKey, apiBaseUrl, connectionMode, onRequestKey, onSessionStateChange, settingsHost, initialPrompt, automationStartToken, automationRetryFailedToken, automationStopToken, onProgressChange, onResultsChange }: {
  ...
  const recommendThemes = async (items: LogoAsset[]) => {
    ...
    setRecommendingSceneIds((current) => [...new Set([...current, ...items.map((item) => item.id)])]);
    let cursor = 0; await Promise.all(Array.from({ length: Math.min(8, items.length) }, async () => { while (cursor < items.length) { const item = items[cursor++]; try { const theme = await recommendSceneTheme({ provider, apiKey: key, apiBaseUrl, geminiModel: config.sceneRecommendationModel, openAiModel: config.openAiSceneRecommendationModel, image: item.file }); setSceneThemes((current) => ({ ...current, [item.id]: theme })); } catch (error) { message.error(`${item.name}：${error instanceof Error ? error.message : '场景推荐失败'}`); } finally { setRecommendingSceneIds((current) => current.filter((id) => id !== item.id)); } } }));
  };
  const addScenes = (files: File[]) => { const next = files.filter((file) => valid(file) === true).map((file) => ({ id: createId(), file, name: file.name, mimeType: file.type, previewUrl: URL.createObjectURL(file) })); if (next.length) { clearResults(); setScenes((current) => [...current, ...next].slice(0, 20)); if (settingsRef.current.autoRecommendScene) { if (!prompt.trim()) setPrompt(SCENE_COMMON_CONSTRAINT); void recommendThemes(next); } } return false; };
  const removeScene = (id: string) => { clearResults(); setSceneThemes((current) => { const next = { ...current }; delete next[id]; return next; }); setScenes((current) => { const found = current.find((item) => item.id === id); if (found) URL.revokeObjectURL(found.previewUrl); return current.filter((item) => item.id !== id); }); };
  const clearScenes = () => { clearResults(); setSceneThemes({}); setScenes((current) => { current.forEach((item) => URL.revokeObjectURL(item.previewUrl)); return []; }); };
  ...
  const start = () => {
    ...
    if (settings.autoRecommendScene && scenes.some((scene) => !sceneThemes[scene.id])) return void message.warning('请等待所有图片完成场景推荐，或手动填写缺少的主题');
    clearResults(); setTasks(scenes.flatMap((scene, sceneIndex) => Array.from({ length: settings.copiesPerScene }, (_, copyIndex) => ({ id: createId(), sceneId: scene.id, sceneIndex, copyIndex, status: 'waiting' as const, prompt: settings.autoRecommendScene ? `${sceneThemes[scene.id]}，${prompt.trim()}` : prompt.trim(), retryCount: 0 }))));
  };
