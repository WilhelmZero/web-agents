...
export default function SceneReplaceComposer({ apiKey, openAiApiKey, apiBaseUrl, connectionMode, onRequestKey, onSessionStateChange, settingsHost }: {
  ...
  return <div className="scene-replace-page"><section className="hero-strip scene-replace-hero"><div><Text className="eyebrow">SCENE REPLACER</Text><Title level={2}>保留主体，只替换场景与氛围</Title><Paragraph className="hero-description">批量改造已有场景图，锁定人物、产品、Logo、文字和构图，让新环境的光线自然融入原图。</Paragraph></div><div className="hero-orb" /></section>
    ...
    <Modal className="scene-result-modal" width={960} title={selectedResult ? `${sanitizeFileName(selectedResult.scene.name)} · ${selectedResultGroup.length} 个生成结果` : '生成结果'} open={Boolean(selectedResult)} footer={<Button onClick={() => setSelectedResultId(undefined)}>关闭</Button>} onCancel={() => setSelectedResultId(undefined)} destroyOnHidden>
      {selectedResult && <Image.PreviewGroup preview={{ onOpenChange: (open) => { if (!open) setPreviewCompareOriginal(false); }, onChange: () => setPreviewCompareOriginal(false), actionsRender: (originalNode) => <>{originalNode}<Tooltip title={previewCompareOriginal ? '查看生成图' : '原图对比'}><button type="button" className={previewCompareOriginal ? 'scene-preview-compare-action is-active' : 'scene-preview-compare-action'} onClick={() => setPreviewCompareOriginal((current) => !current)}><EyeOutlined /></button></Tooltip></>, imageRender: (originalNode) => previewCompareOriginal ? cloneElement(originalNode as ReactElement<{ src?: string; alt?: string }>, { src: selectedResult.scene.previewUrl, alt: '原始场景图' }) : originalNode }}>
        <div className="scene-result-modal-section"><Flex justify="space-between" align="center"><Title level={5}>场景替换结果</Title><Text type="secondary">点击缩略图放大，左右切换全部图片</Text></Flex><div className="scene-result-thumbnail-row">{selectedResultGroup.map(({ task, scene }) => <div className="scene-result-thumbnail-item" key={`scene-${task.id}`}><Image src={task.resultUrl} alt={`场景替换结果 ${task.copyIndex + 1}`} /><Flex justify="space-between" align="center"><Text>结果 {task.copyIndex + 1}</Text><Button size="small" type="primary" icon={task.outpaintBlob ? <ReloadOutlined /> : <ExpandOutlined />} loading={task.outpaintStatus === 'running'} onClick={() => void manualOutpaint([task])}>{task.outpaintBlob ? '重新扩图' : '扩图'}</Button></Flex>{task.outpaintError && <Text type="danger">{task.outpaintError}</Text>}</div>)}</div></div>
        <div className="scene-result-modal-section"><Flex justify="space-between" align="center"><Title level={5}>AI 扩图结果</Title><Text type="secondary">直接显示 AI 返回效果</Text></Flex>{selectedResultGroup.some(({ task }) => task.outpaintUrl) ? <div className="scene-result-thumbnail-row">{selectedResultGroup.filter(({ task }) => task.outpaintUrl).map(({ task, scene }) => <div className="scene-result-thumbnail-item" key={`outpaint-${task.id}`}><Image src={task.outpaintUrl} alt={`AI 扩图结果 ${task.copyIndex + 1}`} /><Flex justify="space-between" align="center"><Text>扩图 {task.copyIndex + 1}</Text><Button size="small" icon={<DownloadOutlined />} onClick={() => task.outpaintBlob && downloadBlob(task.outpaintBlob, fileName(task, scene))}>下载</Button></Flex></div>)}</div> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="尚未生成扩图，使用上方按钮开始扩图" />}</div>
      </Image.PreviewGroup>}
    </Modal>
