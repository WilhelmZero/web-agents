...
export default function SceneReplaceComposer({ apiKey, openAiApiKey, apiBaseUrl, connectionMode, onRequestKey, onSessionStateChange, settingsHost }: {
  ...
  const [resultModalMode, setResultModalMode] = useState<'scene' | 'outpaint'>('scene');
  ...
  const clearResults = () => { aborters.current.forEach((item) => item.abort()); setTasks((current) => { current.forEach((item) => { if (item.resultUrl) URL.revokeObjectURL(item.resultUrl); if (item.outpaintUrl) URL.revokeObjectURL(item.outpaintUrl); }); return []; }); setSelectedResultId(undefined); };
  ...
  const outpaintTask = useCallback(async (taskId: string, source: Blob, signal: AbortSignal) => {
    ...
    try {
      ...
      const finalUrl = URL.createObjectURL(aiBlob);
      setTasks((current) => current.map((item) => {
        if (item.id !== taskId) return item;
        if (item.outpaintUrl) URL.revokeObjectURL(item.outpaintUrl);
        return { ...item, outpaintStatus: 'success', outpaintBlob: aiBlob, outpaintUrl: finalUrl };
      }));
      setResultModalMode('outpaint');
    } catch (error) {
  ...
  return <div className="scene-replace-page"><section className="hero-strip scene-replace-hero"><div><Text className="eyebrow">SCENE REPLACER</Text><Title level={2}>保留主体，只替换场景与氛围</Title><Paragraph className="hero-description">批量改造已有场景图，锁定人物、产品、Logo、文字和构图，让新环境的光线自然融入原图。</Paragraph></div><div className="hero-orb" /></section>
    ...
    <section className="results-section">
      ...
      {tasks.length ? <div className="logo-replace-results scene-replace-results-grid">{resultItems.map(({ task, scene }) => {
        const shownUrl = task.outpaintUrl || task.resultUrl; const downloadable = task.outpaintBlob || task.resultBlob;
        return <Card key={task.id} size="small" title={<Text ellipsis={{ tooltip: scene.name }}>{sanitizeFileName(scene.name)} · 结果 {task.copyIndex + 1}</Text>} extra={downloadable && <Button type="text" icon={<DownloadOutlined />} onClick={() => downloadBlob(downloadable, fileName(task, scene))} />}><button type="button" className="scene-result-preview-trigger" disabled={!shownUrl} onClick={() => { setSelectedResultId(task.id); setResultModalMode(task.outpaintUrl ? 'outpaint' : 'scene'); }}>{shownUrl ? <img src={shownUrl} alt="点击查看场景替换结果" /> : task.status === 'running' ? <GeneratingImage progressKey={task.id} status="running" percent={1} /> : <div className={`task-state-card is-${task.status}`}><Text strong type={task.status === 'failed' ? 'danger' : 'secondary'}>{statusLabel(task.status)}</Text><Text type="secondary">{task.error}</Text></div>}</button><Flex justify="space-between" align="center" style={{ marginTop: 8 }}><Tag color={task.outpaintStatus === 'failed' || task.status === 'failed' ? 'error' : task.outpaintStatus === 'running' || task.status === 'running' ? 'processing' : task.status === 'success' ? 'success' : 'default'}>{task.outpaintStatus === 'running' ? '扩图中' : task.outpaintStatus === 'success' ? '已扩图' : task.outpaintStatus === 'failed' ? '扩图失败' : statusLabel(task.status)}</Tag><Button size="small" icon={<ReloadOutlined />} disabled={task.status === 'running' || task.status === 'waiting' || task.outpaintStatus === 'running'} onClick={() => retry(task)}>重新生成</Button></Flex></Card>;
      })}</div> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="上传图片并开始替换后，结果会显示在这里" />}
    </section>
    <Modal className="scene-result-modal" width={980} title={selectedResult ? `${sanitizeFileName(selectedResult.scene.name)} · 结果 ${selectedResult.task.copyIndex + 1}` : '生成结果'} open={Boolean(selectedResult)} footer={null} onCancel={() => setSelectedResultId(undefined)} destroyOnHidden>
      {selectedResult && <><div className="scene-result-modal-image"><Image src={resultModalMode === 'outpaint' && selectedResult.task.outpaintUrl ? selectedResult.task.outpaintUrl : selectedResult.task.resultUrl} alt="场景替换结果大图" /></div>{selectedResult.task.outpaintUrl && <Segmented block value={resultModalMode} onChange={(value) => setResultModalMode(value as 'scene' | 'outpaint')} options={[{ label: '场景替换结果', value: 'scene' }, { label: 'AI 扩图结果', value: 'outpaint' }]} />}<Flex justify="space-between" align="center" gap={8} wrap style={{ marginTop: 16 }}><Text type={selectedResult.task.outpaintStatus === 'failed' ? 'danger' : 'secondary'}>{selectedResult.task.outpaintError || '扩图直接保留 AI 返回效果，不再合成覆盖原图。'}</Text><Space><Button icon={<DownloadOutlined />} onClick={() => { const blob = selectedResult.task.outpaintBlob || selectedResult.task.resultBlob; if (blob) downloadBlob(blob, fileName(selectedResult.task, selectedResult.scene)); }}>下载当前结果</Button><Button type="primary" icon={selectedResult.task.outpaintBlob ? <ReloadOutlined /> : <ExpandOutlined />} loading={selectedResult.task.outpaintStatus === 'running'} onClick={() => void manualOutpaint([selectedResult.task])}>{selectedResult.task.outpaintBlob ? '重新扩图' : '扩图'}</Button></Space></Flex></>}
    </Modal>
