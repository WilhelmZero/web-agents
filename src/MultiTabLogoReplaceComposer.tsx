...
export function buildLogoPickerFolderTree(groups: FolderGroup[]): PickerFolderNode[] {
...
export default function MultiTabLogoReplaceComposer(props: Props) {
  ...
  const [automationRetryFailedToken, setAutomationRetryFailedToken] = useState<string>();
  ...
  useEffect(() => {
    ...
    setInjected(true);
    channel?.postMessage({ type: 'worker-ready', batchId, groupId, name: workerGroup.name, count: workerGroup.files.length });
  ...
  useEffect(() => {
    ...
    const receive = (event: MessageEvent) => {
      ...
      if (worker && data.type === 'logos-updated' && batchId) void readBatch(batchId).then((batch) => {
        ...
        if (fresh.length) { fresh.forEach((file) => loadedLogoKeys.current.add(`${file.name}:${file.size}:${file.lastModified}`)); message.success(`已同步 ${fresh.length} 个公共 Logo`); }
      });
  ...
  const buildWorkerTarget = (id: string, group: FolderGroup) => {
    ...
    url.searchParams.set('tool', 'logo-replace-tabs'); url.searchParams.set('worker', '1');
    url.searchParams.set('batch', id); url.searchParams.set('group', group.id); url.searchParams.set('folder', group.name);
  ...
  const openWorkers = async () => {
    ...
    if (shouldAnalyzePerImagePromptsInController(perImagePromptEnabled, autoGenerateAfterPromptAnalysis)) { const missing = groups.flatMap((group) => group.files).filter((file) => !perImagePrompts.effective(file)); if (missing.length) { const analyzed = await perImagePrompts.analyze(missing); if (analyzed.failed) { placeholders.forEach((item) => item?.close()); return void message.error(`${analyzed.failed} 张图片提示词分析失败，请重试`); } placeholders.forEach((item) => item?.close()); return void message.success('逐图提示词已分配，请审核后再次打开标签'); } }
  ...
  const openSingleWorker = async (group: FolderGroup) => {
    ...
    if (shouldAnalyzePerImagePromptsInController(perImagePromptEnabled, autoGenerateAfterPromptAnalysis)) { const missing = group.files.filter((file) => !perImagePrompts.effective(file)); if (missing.length) { const analyzed = await perImagePrompts.analyze(missing); if (analyzed.failed) { placeholder?.close(); return void message.error(`${analyzed.failed} 张图片提示词分析失败，请重试`); } placeholder?.close(); return void message.success('逐图提示词已分配，请审核后再次打开标签'); } }
    ...
    else setBlockedWorkerUrls((current) => current.some((item) => item.url === target.url) ? current : [...current, target]);
  };
  const removeGroup = (group: FolderGroup) => {
    setGroups((current) => current.filter((item) => item.id !== group.id));
    setSelectedGroupId((current) => current === group.id ? undefined : current);
    setWorkerProgress((current) => { const next = { ...current }; delete next[group.id]; return next; });
  ...
  const disableCloseWarnings = () => { channel?.postMessage({ type: 'disable-close-warning', batchId: activeBatchId }); message.success('已通知所有工作标签关闭离页提醒'); };
  const retryAllFailedWorkers = () => { if (!activeBatchId || !aggregate.failed) return; channel?.postMessage({ type: 'retry-failed', batchId: activeBatchId, commandId: `retry-${Date.now()}` }); message.success('已通知所有工作标签重试失败任务'); };
  ...
  const reviewFolderFiles = (files: File[]) => {
    ...
    const tree = buildLogoPickerFolderTree(groupFolderFiles(accepted)); const keys: string[] = []; const collect = (nodes: PickerFolderNode[]) => nodes.forEach((node) => { if (node.group) keys.push(`dir:${node.path}`); collect(node.children); }); collect(tree);
    setPendingFolderFiles(accepted); setCheckedFolderKeys(keys);
  };
  ...
  return <div className="multi-tab-logo-page"><section className="hero-strip logo-replace-hero"><div><Text className="eyebrow">MULTI-TAB LOGO REPLACER</Text><Title level={2}>一个主控页，分发多组 Logo 替换任务</Title><Paragraph className="hero-description">一次选择场景根文件夹和公共 Logo，每个最深层子目录自动分配到独立标签页；工作页完整使用现有 Logo 替换功能。</Paragraph></div><div className="hero-orb" /></section>
    ...
    <Modal title="选择要导入的场景文件夹" open={pendingFolderFiles.length > 0} width={980} okText={`导入已选 ${checkedFolderKeys.length} 个目录`} cancelText="取消" okButtonProps={{ disabled: !checkedFolderKeys.length }} onOk={importCheckedFolderFiles} onCancel={() => { setPendingFolderFiles([]); setCheckedFolderKeys([]); }}>
      <Alert type="info" showIcon title="保留完整目录结构，仅选择最深层图片文件夹" description="父级目录只展示结构；叶子目录可以通过复选框或点击整张卡片选择。" style={{ marginBottom: 14 }} />
      <Flex justify="space-between" align="center" style={{ marginBottom: 12 }}><Text type="secondary">已选择 {checkedFolderKeys.length} / {pickerKeys.length} 个最深层目录</Text><Space><Button size="small" onClick={() => setCheckedFolderKeys(pickerKeys)}>全选</Button><Button size="small" onClick={() => setCheckedFolderKeys([])}>取消全选</Button></Space></Flex>
      <div className="scene-folder-picker-tree" style={{ maxHeight: '58vh', overflowY: 'auto', paddingRight: 8 }}>{renderPickerTree(pendingPickerTree)}</div>
    </Modal>
    ...
    <Card className="workflow-card" title="逐图提示词分配"><Flex justify="space-between" align="center"><Text strong>生成前逐图分析</Text><Switch checked={perImagePromptEnabled} onChange={setPerImagePromptEnabled} /></Flex>{perImagePromptEnabled && <><Flex justify="space-between" align="center" style={{ marginTop: 12 }}><Text>由子标签分析并自动生成</Text><Switch checked={autoGenerateAfterPromptAnalysis} onChange={setAutoGenerateAfterPromptAnalysis} /></Flex>{autoGenerateAfterPromptAnalysis ? <Alert style={{ marginTop: 12 }} type="info" showIcon title="主控页不再分析或显示审核列表" description="打开工作标签后，点击一键开始所有替换；每个子标签只分析自己的图片，分析成功后自动进入生成，不占用主控页的分析资源。" /> : <><Button style={{ marginBlock: 12 }} icon={<ReloadOutlined />} onClick={() => void perImagePrompts.analyze()}>分析全部 / 重试失败</Button><div className="per-image-prompt-grid">{groups.flatMap((group) => group.files.map((file) => <Card size="small" key={`${group.id}-${perImagePromptFileKey(file)}`} title={`${group.name} · ${file.name}`}><PerImagePromptEditor file={file} assignment={perImagePrompts.assignments[perImagePromptFileKey(file)]} sourcePrompt={logoPromptSource} onEdit={(value) => perImagePrompts.edit(file, value)} onAnalyze={() => void perImagePrompts.analyze([file])} /></Card>))}</div></>}</>}</Card>
