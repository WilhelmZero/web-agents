...
export default function ObjectReplaceComposer({ apiKey, apiBaseUrl, connectionMode, onRequestKey, onSessionStateChange, settingsHost }: {
  ...
  const panel = <div className="settings-panel object-replace-settings-panel">
    ...
    <Form layout="vertical" style={{ marginTop: 20 }}>
      <Form.Item label="严格保留新物体元素"><Space direction="vertical">{ELEMENTS.map((item) => <Checkbox key={item.key} checked={settings.preservation[item.key]} onChange={(event) => preserve({ [item.key]: event.target.checked })}>{item.label}</Checkbox>)}</Space>
