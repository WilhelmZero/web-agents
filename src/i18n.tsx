...
const translations: Record<string, string> = {
  ...
  '先按像素确定杯子的位置、尺寸和白底画布，再让 AI 只完成自然融合。': 'Set the exact cup position, size, and clean-image canvas first, then let AI handle only the natural blending.',
  ...
  '先涂抹挡住原杯子，再切换移动工具放置新杯子': 'Paint over the original cup, then switch to Move to position the new cup',
  '杯子白底图始终位于最上层；画布放大超过原图时会用自动识别的底色填充。': 'The clean cup image always stays on top. Enlarged canvas areas use the automatically detected background color.',
  '移动杯子': 'Move cup',
  '画笔': 'Brush',
  '方形填充': 'Rectangle fill',
  '椭圆填充': 'Ellipse fill',
  ...
  '严格锁定指导图中的杯子位置、大小与轮廓': 'Strictly lock the cup position, size, and silhouette from the guide',
  '开始融合': 'Start blending',
  '融合设置': 'Blending settings',
  '精确尺寸': 'Exact size',
  '位置和大小来自指导合成图': 'Position and size come from the composite guide',
  'AI 只负责移除白底并补充自然光影、遮挡和边缘融合，不应重新设计杯子。': 'AI only removes the clean background and adds natural lighting, occlusion, and edge blending; it should not redesign the cup.',
  ...
  '正数裁切，负数向外扩展并填充识别到的底色': 'Positive values crop; negative values extend and use the detected background color.',
  '上': 'Top',
  '右': 'Right',
  '下': 'Bottom',
  '左': 'Left',
  ...
  '杯子的最终宽高和位置严格来自指导合成图；涂抹区域只用于恢复旧杯子后方的场景。': 'The final cup dimensions and position come strictly from the composite guide; painting is only used to restore the scene behind the old cup.',
  '补充提示词': 'Additional prompt',
  '核心提示词': 'Core prompt',
  '可自定义 AI 融合规则；留空时使用默认提示词。': 'Customize the AI blending rules. Leave blank to use the default prompt.',
  '输入杯子尺寸与场景融合的核心提示词': 'Enter the core prompt for cup sizing and scene blending',
  '可留空；例如：给杯子倒上半杯酒': 'Optional; for example: fill the cup halfway with wine',
  '补充要求会追加到严格尺寸约束之后；留空时只执行杯子与场景的自然融合。': 'Additional instructions are appended after the strict size constraints. Leave blank to perform only natural scene blending.',
  '放大查看': 'Open preview',
  '指导图': 'Guide',
  '矩形填充': 'Rectangle fill',
  '撤销涂抹': 'Undo paint',
  ...
  '可以关闭此窗口，完成后会自动替换原生成图片。': 'You can close this window. The original generated image will be replaced automatically when processing finishes.',
  '关闭窗口': 'Close window',
  ...
  '去除 Logo': 'Remove Logo',
  '批量识别并自然去除产品表面的 Logo': 'Batch-detect and naturally remove logos from product surfaces',
  '批量去除杯身 Logo': 'Batch remove logos from cups',
  '多个文件夹在一个页面统一分析、生成、校验和重试，不创建子标签。': 'Analyze, generate, verify, and retry multiple folders in one page without opening child tabs.',
  ...
  '结果图片已隐藏，任务仍在后台正常执行': 'Result images are hidden while tasks continue running',
  ...
  '自动按原目录分组，支持 PNG、JPEG、WebP': 'Automatically group by source folder; supports PNG, JPEG, and WebP',
  ...
  '按文件夹展示，点开后按需加载全部生成图': 'Grouped by folder. Open a folder to load all generated images on demand.',
  ...
  '其他': 'Other',
  '填写需要去除 Logo 的载体或区域，例如：皮革收纳盒正面': 'Describe the surface or area whose Logo should be removed, e.g. the front of a leather storage box',
  ...
  '网页刷新后不会自动恢复旧任务；需要时请点击“恢复缓存任务”。': 'Old tasks are not restored automatically after refresh. Use “Restore cached tasks” when needed.',
  ...
  '在左侧工具名称旁点击置顶图标，常用工具就会显示在这里。': 'Click the pin beside a tool in the sidebar to add it here.',
  '批量生成风格统一的商业场景图': 'Generate consistent commercial scene images in batches',
  '保留主体姿态并替换主题与环境': 'Replace the theme and environment while preserving the subject pose',
  '精确控制杯子的位置、尺寸与融合': 'Precisely control cup position, size, and blending',
  '将品牌标识自然合成到产品图片': 'Blend brand marks naturally into product images',
  '批量识别、替换并校验场景 Logo': 'Detect, replace, and verify scene logos in batches',
  '跨标签并行执行多组 Logo 替换任务': 'Run multiple logo replacement batches across tabs',
  '从图片或 PSD 图层批量整理 Logo': 'Organize logos from images or PSD layers in batches',
  '识别并批量修改花纸中的文字内容': 'Detect and batch-edit text in artwork',
  '智能抠图、透明化并支持矢量输出': 'Remove backgrounds with transparent and vector output',
  '按目标比例自然补全图片边界': 'Naturally extend image boundaries to a target ratio',
  '批量替换画面中的指定物体': 'Replace selected objects across images in batches',
  '框选局部区域并进行精细重绘': 'Select local regions for precise repainting',
  '组合素材生成商品详情长图': 'Compose assets into long-form product detail images',
  '查看 Gemini 免费余额': 'View Gemini free balance',
  '查看 OpenAI 账单与额度': 'View OpenAI billing and limits',
  '将在新标签页打开官方账户页面': 'The official account page will open in a new tab',
  '浏览器已登录对应账号时可直接查看；出于官方页面的安全限制，余额无法嵌入或由普通 API Key 自动读取。': 'If the relevant account is already signed in, the page opens directly. Official security restrictions prevent embedding the balance or reading it with a regular API key.',
  ...
  '跨标签并行执行多组场景替换与扩图任务': 'Run grouped scene replacement and outpainting tasks across tabs',
  '一个主控页，分发多组场景替换任务': 'One controller for multiple scene replacement groups',
  ...
  '可以勾选或取消整个文件夹，也可以展开目录后逐张选择图片；不会修改电脑中的原文件。': 'Select or clear whole folders, or expand them to choose individual images. Files on your computer are not modified.',
  '预览底色': 'Preview background',
  'Logo 预览底色': 'Logo preview background',
  '透明棋盘格': 'Transparent checkerboard',
  '白底': 'White',
  '黑底': 'Black',
  '只改变 Logo 缩略图的查看底色，不会修改或导出到图片中。': 'Only changes the Logo thumbnail preview background. It is not applied to or exported with the image.',
  ...
  '移除全部文件夹？': 'Remove all folders?',
  '只清空当前批次中的文件夹，不会删除电脑中的原文件。': 'Only clears folders from the current batch. Original files on your computer are not deleted.',
  '移除全部 Logo': 'Remove all Logos',
  '移除全部 Logo？': 'Remove all Logos?',
  '将同时清空公共新 Logo 和旧 Logo 参考图，不会删除电脑中的原文件。': 'Clears all shared new Logos and the old Logo reference. Original files on your computer are not deleted.',
  '已移除全部文件夹分组': 'All folder groups removed',
  '已移除全部 Logo': 'All Logos removed',
  '只从当前批次移除，不会删除电脑中的文件。': 'Only removes it from this batch. Files on your computer are not deleted.',
  ...
  '工作流': 'Workflow',
  '连接创作工具节点，批量预测并自动执行': 'Connect creation tools, predict batches, and run automatically',
  ...
  '拖入画布或点击添加。输出可连接多个下游，输入端口只能连接一个上游。': 'Drag onto the canvas or click to add. Outputs may branch; each input accepts one upstream.',
  '配置': 'Settings',
  '输入': 'Input',
  '输出': 'Output',
  '运行日志': 'Run log',
  ...
  '开启后允许一张 Logo 循环匹配全部场景': 'Allows one logo to be reused across every scene',
