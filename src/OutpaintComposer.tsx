import { ClearOutlined, DeleteOutlined, DownloadOutlined, ExpandOutlined, FileImageOutlined, ReloadOutlined, RocketOutlined, StopOutlined } from '@ant-design/icons';
import { App, Button, Card, Empty, Flex, Form, Image, Input, InputNumber, Popconfirm, Progress, Segmented, Select, Space, Tag, Typography, Upload } from 'antd';
...
interface Item { id: string; file: File; sourceUrl: string; status: 'waiting' | 'running' | 'success' | 'failed' | 'stopped'; aiResultBlob?: Blob; aiResultUrl?: string; resultBlob?: Blob; resultUrl?: string; error?: string }
...
export default function OutpaintComposer({ apiKey, openAiApiKey, apiBaseUrl, connectionMode, onRequestKey, onSessionStateChange, settingsHost }: {
  ...
  const processItem = async (item: Item, signal: AbortSignal) => {
    ...
    try {
      ...
      const aiResultUrl = URL.createObjectURL(generated);
      if (item.aiResultUrl) URL.revokeObjectURL(item.aiResultUrl); if (item.resultUrl) URL.revokeObjectURL(item.resultUrl); patchItem(item.id, { status: 'success', aiResultBlob: generated, aiResultUrl, resultBlob, resultUrl }); setPreviewModes((current) => ({ ...current, [item.id]: 'result' }));
  ...
  const clearAll = () => { aborter.current?.abort(); setItems((current) => { current.forEach((item) => { URL.revokeObjectURL(item.sourceUrl); if (item.aiResultUrl) URL.revokeObjectURL(item.aiResultUrl); if (item.resultUrl) URL.revokeObjectURL(item.resultUrl); }); return []; }); setPreviewModes({}); };
  ...
  const previewUrl = (item: Item) => { const mode = previewModes[item.id] || 'result'; return mode === 'source' ? item.sourceUrl : mode === 'ai' ? item.aiResultUrl : item.resultUrl; };
  ...
  return <div className="outpaint-page"><section className="hero-strip outpaint-hero"><div><Text className="eyebrow">AI OUTPAINT</Text><Title level={2}>完整保留原图，智能补齐画面之外</Title><Paragraph className="hero-description">按指定比例或精确分辨率扩展画布，只补充缺失环境，并在最终结果中原位覆盖回原图。</Paragraph></div><div className="hero-orb" /></section>
    ...
    <section className="results-section">
      <Flex justify="space-between" align="center" wrap gap={8}><div><Title level={3}>扩图结果</Title><Text type="secondary">可切换查看原图、AI 未合成返回图和最终合成图</Text></div><Button icon={<DownloadOutlined />} disabled={!successful.length} onClick={() => void downloadAll()}>下载全部 ZIP</Button></Flex>
      {items.length ? <Image.PreviewGroup><div className="outpaint-results-grid">{items.map((item) => <Card key={item.id} size="small" title={item.file.name} extra={(item.aiResultBlob || item.resultBlob) && <Space size={4}>{item.aiResultBlob && <Button type="text" onClick={() => downloadBlob(item.aiResultBlob!, `${sanitizeFileName(item.file.name)}_AI扩图返回.png`)}>AI 返回</Button>}{item.resultBlob && <Button type="text" icon={<DownloadOutlined />} onClick={() => downloadBlob(item.resultBlob!, fileName(item))}>最终图</Button>}</Space>}>
        ...
        {(item.aiResultUrl || item.resultUrl) && <Segmented block size="small" style={{ marginTop: 8 }} value={previewModes[item.id] || 'result'} onChange={(mode) => setPreviewModes((current) => ({ ...current, [item.id]: mode as PreviewMode }))} options={[{ label: '原图', value: 'source' }, ...(item.aiResultUrl ? [{ label: 'AI 返回', value: 'ai' }] : []), ...(item.resultUrl ? [{ label: '最终合成', value: 'result' }] : [])]} />}
