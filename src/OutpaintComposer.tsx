...
export default function OutpaintComposer({ apiKey, openAiApiKey, apiBaseUrl, connectionMode, onRequestKey, onSessionStateChange, settingsHost }: {
  ...
  const processItem = async (item: Item, signal: AbortSignal) => {
    ...
    try {
      ...
      const resultUrl = URL.createObjectURL(generated);
      if (item.resultUrl) URL.revokeObjectURL(item.resultUrl); patchItem(item.id, { status: 'success', resultBlob: generated, resultUrl }); setPreviewModes((current) => ({ ...current, [item.id]: 'result' }));
  ...
  const clearAll = () => { aborter.current?.abort(); setItems((current) => { current.forEach((item) => { URL.revokeObjectURL(item.sourceUrl); if (item.resultUrl) URL.revokeObjectURL(item.resultUrl); }); return []; }); setPreviewModes({}); };
