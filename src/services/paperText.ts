...
import type { OpenAiImageOutputSize } from "../types";
...
export function normalizePaperTextRegions(value: unknown): PaperTextRegion[] {
  ...
  return regions.flatMap((item) => {
    ...
    const rawBox = usesArrayBox
      ? (region.box as unknown[])
      : [region.left, region.top, region.width, region.height];
    if (
      typeof region.text !== "string" ||
      !region.text.trim() ||
      rawBox.some((part) => !Number.isFinite(Number(part)))
    )
      return [];
    const scale =
      !usesArrayBox && rawBox.some((part) => Number(part) > 100) ? 0.1 : 1;
    const box = rawBox.map((part) =>
      Math.max(0, Math.min(100, Number(part) * scale)),
    ) as PaperTextRegion["box"];
    return [{ original: region.text, text: region.text, box }];
  });
}

export function buildPaperTextEditPrompt(
  ...
  const lines = changes
    .map(
      (region, index) =>
        `${index + 1}. 在百分比区域 [${region.box.join(", ")}]，将“${region.original}”准确替换为“${region.text}”`,
    )
    .join("\n");
...
async function requestOpenAiJson(options: {
  apiKey: string;
  model: string;
  image: File | Blob;
  prompt: string;
  ...
  const consoleId = startRequestConsoleEntry({
    model: options.model,
    connection: "direct",
    requestSummary: "OpenAI Responses · 1 张输入图片 · 结构化文字结果",
  ...
  try {
    ...
    try {
      ...
      throw new Error("OpenAI 返回的文字结果格式无效");
    }
    updateRequestConsoleEntry(consoleId, {
      status: "success",
      httpStatus,
      durationMs: Math.round(performance.now() - startedAt),
      resultSummary: "结构化文字结果",
      message: "GPT 请求完成",
    });
    return value;
  } catch (error) {
...
export async function recognizePaperTextOpenAi(options: {
  apiKey: string;
  model: string;
  image: File;
  signal?: AbortSignal;
  ...
  const value = await requestOpenAiJson({
    ...options,
    prompt:
      "识别图片中所有可见的包装花纸文字。按独立文字区域分组，严格保持原文、大小写、标点和换行。box 为 [左侧百分比, 顶部百分比, 宽度百分比, 高度百分比]，范围 0-100。不要把纯图形或不可见的猜测内容当作文字。",
    schema: regionsSchema,
  });
  return normalizePaperTextRegions(value);
}

export async function editPaperTextOpenAi(options: {
  apiKey: string;
  model: string;
  image: File;
  mask?: File;
  ...
  if (supportsOpenAiInputFidelity(options.model))
    form.append("input_fidelity", "high");
  const startedAt = performance.now();
  const consoleId = startRequestConsoleEntry({
    model: options.model,
    connection: "direct",
    requestSummary: `OpenAI Images Edit · 1 张输入图片 · ${options.quality} 质量`,
...
export async function verifyPaperTextOpenAi(options: {
  apiKey: string;
  model: string;
  image: Blob;
  regions: PaperTextRegion[];
