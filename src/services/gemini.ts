...
export function buildLogoReplacementInstruction(options: {
  ...
  const ribbedBeerMugInstruction = options.beerMugContext ? '【啤酒杯专项补充】仅因本图已识别为啤酒杯而启用：若杯身上部为连续光滑区域、下部为竖纹、棱柱、浮雕、切面或凹凸花纹区域，先检测下部纹理开始线。新 Logo 必须保持参考图原始宽高比并等比收纳在旧 Logo 所在的上部光滑区域，宁可缩小并留白，也不得裁切、拉伸、向下进入纹理区或移动到杯身中下部。旧 Logo 若贴近泡沫线或液面线，新 Logo 必须保持相同高度关系。此补充不得应用于非啤酒杯载体。' : '';
