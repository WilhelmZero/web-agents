...
export function buildObjectReplacementInstruction(options: {
  ...
  const uniformScaleInstruction = '【最高优先级：严格同比例缩放】新物体本体只能进行刚性变换与各轴相同倍率的统一缩放。缩放系数必须满足 Sx = Sy = Sz；投影到画面前，新物体的宽高比、长宽比、各高度截面宽度占总宽的比例、轮廓控制点相对坐标、曲率、锥度和局部厚度比例必须与新物体参考图完全一致。严禁单轴缩放、自由变换、液化、瘦身、增宽、压扁、拉长、局部收缩、局部膨胀或为了适配旧物体遮罩而变形。若按原计划尺寸无法放入场景，只能减小或增大同一个统一缩放倍率并保持接触点和中心轴，不得改变任何局部比例。相机透视可以造成自然的视觉缩短，但不得借透视之名修改物体自身几何。输出前必须将生成物体的归一化轮廓与新物体参考图逐段比较；任何杯口、杯肚、腰线、杯底或其他轮廓比例偏差都判定为不合格，必须恢复新物体原始比例后再输出。';
  const cupInstruction = isCup
    ? '【杯子本体专项】把“杯子本体”限定为实体杯身外壳、杯口、杯底以及新杯本身不可分离的把手；只从新参考图采用这些部分的几何结构、轮廓、透明度或不透明度、材质、基础颜色，以及已勾选的杯身表面特征。杯内液体、泡沫、冰块，以及场景中的吸管、可拆卸杯盖、装饰挂件、手和手指默认都不属于新杯本体；其来源必须严格服从前述勾选规则：未勾选时沿用原场景，只有明确勾选的元素才允许从新参考图移植。所有这些元素都只按新杯口和杯身做最小限度的自然遮挡适配。保持原场景杯底接触点、杯身中心轴线、整体旋转、相机视角、手部握持点和前后遮挡顺序，但杯子的几何外形只能来自新杯参考图。必须先测量并锁定新杯正面轮廓的归一化几何特征：总高与最大宽度之比、杯口宽度、杯肚最宽处、腰部最窄处、杯底宽度、各高度截面的宽度变化曲线、侧壁曲率、锥度、杯口厚度和杯底厚度。以新杯高度为单一缩放基准做等比缩放，宽度必须由新杯原始宽高比自动决定；只允许统一缩放、旋转和透视投影，严禁任何横向或纵向非等比拉伸，严禁为了适配旧杯宽度而改变腰线、曲线、锥度或杯肚宽度。'
    : '';

  return `执行“仅替换物体本体”的严格局部编辑任务。${references.join('')} 第一步，识别原始场景中所有符合“${sourceName}”的实例，全部实例都必须替换，不得遗漏，并为每个实例分离出实体本体区域；不得把其内部内容物、前景遮挡物、手部、可拆卸附件或背景算入本体区域。第二步，以旧物体的底部接触点、中心轴线和可见高度定位新物体，用“${targetName}”的新物体本体替换；允许修改区域为旧物体轮廓与按新物体真实比例放置后的新轮廓之并集，不得用旧物体遮罩边界限制或裁切新物体宽度；新轮廓超出旧轮廓时只在必要的邻近区域恢复正确遮挡，新轮廓小于旧轮廓时必须依据周围像素自然补回原背景；新物体本体必须保持参考图或名称定义的结构、轮廓、比例、材质和基础颜色，但必须重新按照原场景的 Position、整体 Scale、Rotation、Perspective、Camera Angle、接地关系、景深和遮挡关系进行渲染，绝不能复制新物体参考图的拍摄视角和摆放方式。${surfaceInstruction}${contentRules}${uniformScaleInstruction}${cupInstruction} 原场景是除新物体本体身份之外所有信息的最高优先级来源。人物、手部、背景、桌面、道具、文字、内容物、附件、构图、裁切、镜头、光线、颜色和清晰度必须与原场景一致。原有阴影、反射、折射和高光应尽量保留，只允许在紧贴新物体表面的狭小范围内做符合新几何形状的必要融合，禁止重做整张图的光影。不得新增、删除或移动任何非目标元素，不得改变画面布局，不得把目标参考图的环境带入结果。输出前逐项检查：变化必须仅限于旧物体轮廓、新物体真实比例轮廓及其紧邻融合边缘；若背景、手、液体、泡沫、吸管、杯盖、道具或构图发生变化，则结果不合格，必须恢复为原场景。`;
}
