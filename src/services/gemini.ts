...
export function buildLogoReplacementInstruction(options: {
  ...
  const ribbedBeerMugInstruction = '【上部光滑带、下部竖纹啤酒杯专项】若载体是带把啤酒杯、马克杯或玻璃杯，并且杯身上部为连续光滑可雕刻区域、下部为竖纹、棱柱、浮雕、切面或凹凸花纹区域，必须先检测并锁定“下部竖纹/浮雕开始线”，把它作为新 Logo 绝对不可越过的下边界。旧 Logo 位于上部光滑带时，新 Logo 的全部文字、外框、装饰、尖角和笔画必须完整收纳在同一上部光滑带内，底边与竖纹开始线保留清晰安全间距，任何像素都不得进入、覆盖或跨越下部纹理区。新 Logo 必须保持原始宽高比，以旧 Logo 中心和原高度分区为基准等比缩小；若新 Logo 比旧 Logo 更高、装饰更多或纵横比不同，只能同时缩小宽高，禁止固定上端后向下延伸，禁止为了展示完整图案而占满杯身。宁可明显缩小，也不得触碰竖纹开始线；严禁把 Logo 放到杯身中段、竖纹上或杯底附近。';
