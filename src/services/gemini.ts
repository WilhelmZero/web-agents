...
export function buildLogoReplacementInstruction(options: {
  ...
  const microTextInstruction = '【新 Logo 图形拓扑冻结】新 Logo 必须作为完整的不可拆分图形资产进行像素级外观复制，尤其是尺寸很小的文字、字母、数字、标点、细线、轮廓和负空间。参考 Logo 中所有镂空、留白、孔洞、透明区域、内外轮廓及被轮廓包围的载体可见区域都属于 Logo 拓扑本身，必须保持完全相同：镂空区域必须继续显示下方载体材质，严禁填成白色、黑色、木色或任何实心色块，严禁封闭开口、合并孔洞、加粗后吃掉负空间或把轮廓图改成填充徽章。严禁对 Logo 执行 OCR 后重新输入、拼写、翻译、纠错、补字、猜字、改写字体或生成相似字母；不得把任何字符替换为其他字符，不得产生乱码。必须保持参考 Logo 中每个字符的数量、顺序、大小写、字形轮廓、间距、基线、粗细和相对位置完全一致。即使小字无法语义识别，也必须把原始笔画当作图形纹理逐笔保留，而不是解释其文字含义。';
  ...
  const correctionInstruction = options.correctionFeedback?.trim()
    ...
    : '';
  const placementLockInstruction = '【旧 Logo 定位遮罩锁定，最高优先级】在生成前先逐个分割原始场景中的旧 Logo，并记录每个旧 Logo 在整张图中的归一化包围框 left/top/right/bottom、中心点 cx/cy、文字基线、旋转角度以及相对载体边缘的位置。还必须记录它与杯口、泡沫线、液面线、杯把、杯身花纹起始线、杯底和载体外轮廓等最近地标的上下左右距离。新 Logo 只能在对应旧 Logo 的原定位遮罩内进行等比适配：中心点相对载体的偏移误差不得超过载体可见宽高的约 1.5%，包围框各边不得无故漂移超过约 3%；不得把位于杯身上半部、泡沫线或液面线附近、花纹上方平滑区的旧 Logo 下移到杯子几何中心或视觉中心，也不得以“更美观、居中、对称、完整展示”为理由重新排版。对于带把啤酒杯或马克杯，若旧 Logo 位于杯口下方的上半部并跨越或贴近泡沫/液面分界，新 Logo 必须仍停留在相同高度和同一分界附近，绝不能移到杯身中段或下段。新旧 Logo 宽高比不同时，只能保持新 Logo 原始宽高比并在旧包围框内居中等比缩放，禁止移动中心点来腾出空间。除高脚杯专项或杯底安全区明确要求修正明显越界的旧 Logo 外，旧 Logo 的中心点、上下高度、所属表面分区和外包围区域均是不可修改参数。';
  const surfaceConformanceInstruction = '【杯体及曲面贴合强制规则】旧 Logo 只用于确定替换目标及上述不可移动的定位遮罩；其图形内容、平面形状或错误曲面变形不得作为新 Logo 的外观参考。若旧 Logo 位于杯子、玻璃杯、保温杯、瓶子或任何弧形载体上，必须在完全不移动定位遮罩中心点和所属高度分区的前提下，从杯身轮廓、杯口与杯底椭圆、侧壁收放曲线、杯身中心轴线、局部表面法线和相机视角重新估算该位置真实的圆柱形、锥形或不规则三维曲率，再把新 Logo 当作附着于该三维表面的二维纹理进行 UV 投影，投影输入必须是新 Logo 原始平面图。新 Logo 必须沿杯体真实横向曲率连续包裹，并随离开正面中心的角度增大而使左右两侧逐渐横向压缩；左右两侧的压缩量应由杯体截面半径和视角决定，而不是照抄旧 Logo。其上下边缘、文字基线、字符、图形和每条笔画必须共享同一连续曲面映射，不得保持平面矩形，也不得对不同局部单独拉伸。曲面投影只能改变新 Logo 在锁定区域内的物理投影视形，不能改变 Logo 的字符内容、图形拓扑、笔画数量、相对布局、中心区域比例或把整个 Logo 搬到载体中央。最终两侧收缩、旋转、透视缩短和杯体边缘遮挡必须以该原位置的杯面几何为准，不得跨出杯体轮廓或锁定包围区域。同步继承杯体上的高光、阴影、透明度、反射、折射和材质颗粒，使这些光学效果连续穿过 Logo。禁止平面贴纸感、悬浮感、正视图硬贴、左右宽度不收缩、边缘翘起、照抄错误旧弧度或与杯体真实曲率不一致。输出前沿 Logo 上、中、下三条水平带分别检查其曲率和两侧压缩是否与对应高度的杯身截面一致；若任何笔画看起来是平的、浮在杯子前方、沿用了旧 Logo 的错误弧度或整体位置发生漂移，必须在原定位遮罩内重新投影后再输出。';
  ...
  const ribbedBeerMugInstruction = '【上部光滑带、下部竖纹啤酒杯专项】若载体是带把啤酒杯、马克杯或玻璃杯，并且杯身上部为连续光滑可雕刻区域、下部为竖纹、棱柱、浮雕、切面或凹凸花纹区域，必须先检测并锁定“下部竖纹/浮雕开始线”。以旧 Logo 包围框宽 W、高 H 计算最终安全框：左、右各内缩约 0.04W，顶部内缩约 0.04H；安全底边取“旧框 bottom - 0.08H”和“纹理开始线 - 0.06H”中的较小值。该安全框是新 Logo 全部可见像素的绝对边界，不是位置建议；新 Logo 的所有文字、外框、装饰、尖角和笔画都必须完整包含其中。必须采用 CSS contain 同等逻辑做统一等比缩放：scale = min(安全框宽/新 Logo 原始宽, 安全框高/新 Logo 原始高)，只能同时缩小宽高，然后把缩放后的完整 Logo 居中放入安全框。不得 cover、不得裁切、不得非等比变形、不得为了填满安全框而分别拉伸宽高；新 Logo 比旧 Logo 更高或装饰更多时，必须明显缩小并允许左右留出大量空白，禁止固定顶部后向下延伸。输出前必须读取最终 Logo 外包围框并逐边核对：left≥安全框 left、top≥安全框 top、right≤安全框 right、bottom≤安全框 bottom；任一条件不满足都必须继续缩小后再输出。宁可 Logo 看起来偏小，也不得触碰或进入竖纹区、杯身中段或杯底附近。';
  const cupBottomInstruction = '【杯底 Logo 专项安全区】若原 Logo 位于杯底、杯底外侧、底面圆盘或俯视可见的圆形底部，必须先识别杯底最内层连续、平坦且可雕刻/印刷的安全区域及其同心边界。新 Logo 必须整体等比缩放并完整收纳在该内层安全区中央，Logo 最外侧与安全区边界至少保留约 10%–15% 的净空；任何文字、笔画、装饰、尖角和外框都不得跨过杯底内圈、同心装饰环、折射高光分界、倒角、外缘或侧壁，不得被杯底四周裁断。杯底透视为椭圆时需整体随同一平面透视压缩，禁止局部拉伸来塞满边缘；宁可明显缩小，也绝对不得超出杯底。';
  ...
  const immutableSceneInstruction = '【除旧 Logo 外整图冻结，最高优先级】先锁定旧 Logo 的每一个原始像素区域，只允许在这些区域内清除旧 Logo 并重建新 Logo；区域外的场景必须与原图完全一致。杯子、瓶子、礼盒、木盒、盒盖、内衬、人物、手和所有道具的外形、数量、大小、角度、朝向、位置、透视、裁切、遮挡、接触和承托关系都不得改变。原图中平放、侧放、倾斜或嵌在盒内内衬中的杯子必须保持完全相同的姿态和接触位置，严禁立起、悬浮、漂移、旋转、重新摆放或离开凹槽。新 Logo 必须继承旧 Logo 的前后层级和遮挡蒙版：若旧 Logo 被手、手指、杯沿、反光或其他物体挡住，新 Logo 的相同部分也必须位于遮挡物后方，严禁穿透遮挡或贴到手、手指、皮肤及前景物体表面。只替换原图实际存在的旧 Logo；任何原本没有旧 Logo 的表面都必须保持无 Logo，尤其禁止在盒子外侧、盒盖、盒子内部、内衬、锁扣、铰链及空白杯面上自行新增、复制或延伸 Logo。';
  return `执行严格的 Logo 替换任务。第一张图片是原始场景图，${referenceInstruction}${immutableSceneInstruction}${placementLockInstruction}${colorInstruction}${effectInstruction}${microTextInstruction}${exactTextInstruction}${correctionInstruction}${surfaceConformanceInstruction}${stemwareInstruction}${ribbedBeerMugInstruction}${cupBottomInstruction}${antiOverlayInstruction}${zeroColorFinalCheck} 只允许改变旧 Logo 定位遮罩覆盖的区域：保持每个 Logo 原有中心点、上下高度、外包围区域、遮挡关系和材质融合方式，严禁把 Logo 自动移到杯子或载体中间。只有高脚杯专项、上部光滑带专项或杯底安全区明确指出需要缩小以避免越界时，才允许在对应安全区内做最小幅度修正。角度、透视和曲面包裹必须依据原定位处的载体真实几何重新计算，不能照抄可能错误的旧 Logo 形变，并用新 Logo 准确替换。若同一场景存在多个旧 Logo，必须全部替换。除 Logo 外，原图所有像素对应内容必须保持不变，包括画幅、构图、裁切、镜头、主体、产品结构、杯体、背景、人物、道具、已有非 Logo 文字、颜色、光线、阴影、反射、折射、景深、噪点和清晰度。不得移动、删除、增加、重绘或重新设计任何非 Logo 内容，不得在原本没有 Logo 的位置新增 Logo。`;
}
...
export async function verifyLogoReplacement(options: {
  ...
  const data = await postGemini(options.model, options.apiKey, {
    contents: [{ role: 'user', parts: [
      { text: `第一张图片是新 Logo 参考，第二张图片是替换前的原始场景，第三张图片是生成后的场景。执行严格的 Logo 替换验收，以下各项必须全部通过才允许 passed=true：一、新 Logo 的字符、图形、比例、内外轮廓、镂空、孔洞和负空间与参考一致，参考中镂空处必须继续显示载体，任何镂空被填实、孔洞封闭或轮廓变成实心徽章都必须 graphicConsistent=false、passed=false；二、原 Logo 已完整移除，没有新旧 Logo 重叠或残影；三、新 Logo 真实贴合原 Logo 所在载体的曲面、透视、折射、反射、纹理、凹凸、环境光、景深和颗粒；四、必须先分别测量原图旧 Logo 与生成图新 Logo 相对同一载体的归一化包围框、中心点、文字基线、旋转角以及它们相对杯口、泡沫线、液面线、杯把、杯身花纹起始线和杯底的位置。除高脚杯、上部光滑带或杯底专项安全修正外，新 Logo 中心点偏移超过载体可见宽高约 1.5%、任一包围框边漂移超过约 3%、所属上下高度分区改变，或把原本位于杯身上半部/泡沫液面附近的 Logo 移到杯子中间或下方，都必须 placementConsistent=false、passed=false；若位于杯底，全部笔画必须位于杯底最内层平坦安全区，四周至少保留约 10%–15% 净空，任何部分跨出内圈、装饰环、倒角、外缘或侧壁都必须 placementConsistent=false、passed=false；五、除原 Logo 区域外，杯子、盒子、内衬、人物、手、背景和道具的形状、位置、角度、姿态、遮挡、接触及承托关系与原图完全一致；六、没有在任何原本无旧 Logo 的位置新增 Logo。重点检查带把啤酒杯或马克杯：旧 Logo 若位于杯口下方上半部并跨越或贴近泡沫/液面分界，生成 Logo 必须仍在相同高度和同一分界附近，移到杯身视觉中心即判失败。若杯身是上部光滑、下部竖纹/棱柱/浮雕结构，必须从原图旧 Logo 框计算最终安全框：左右各内缩旧框宽约 4%，顶部内缩旧框高约 4%，底边取“旧框底边上缩约 8% 旧框高”和“竖纹开始线上移约 6% 旧框高”中更靠上的值。生成图新 Logo 的全部可见像素必须逐边包含在该安全框内；必须采用 contain 等比缩放，不能 cover、裁切、分别拉伸宽高或固定顶部向下展开。任何一边越过安全框、底边进入或贴住竖纹区、相对旧 Logo 向下扩张、或本可等比缩小却占满杯身时，必须 placementConsistent=false、passed=false，并在 differences 中写出原框、纹理线、计算后的安全框和生成框坐标，明确要求按 contain 继续缩小。重点检查杯子原本平放或嵌在盒内却变成立起、悬浮、移动、旋转或离开内衬的情况；出现任一情况必须 placementConsistent=false、passed=false。若旧 Logo 原本被手或手指挡住，新 Logo 必须仍在手后方；Logo 穿过手、覆盖手或贴在皮肤上时必须 materialIntegrated=false、placementConsistent=false、passed=false。盒子外侧、盒盖、盒子内部、内衬、锁扣或铰链原本没有旧 Logo 却出现新 Logo 时必须 placementConsistent=false、passed=false，并在 differences 中明确指出。${expected ? `准确文字必须逐字符等于〈${expected}〉，包括大小写、空格、数字和标点。` : '未提供准确文字，请直接比较参考 Logo 与生成 Logo 的全部可见字符、笔画、布局和图形。'}重点识别“直接覆盖”：如果新 Logo 像平面贴纸、水印或图层一样叠在场景上，必须设置 flatOverlayDetected=true、materialIntegrated=false、passed=false。对于高脚杯、葡萄酒杯、香槟杯和鸡尾酒杯，Logo 必须位于杯肚上半部，宽度不超过杯肚最大宽度约 42%，不得接近杯梗或底座；过大或偏下必须 placementConsistent=false、passed=false。任何字符错误、图形拓扑或镂空变化、旧 Logo 残留、非 Logo 内容变化、新增 Logo、遮挡错误、平面覆盖、材质未融合或位置尺寸异常都判定失败。differences 数组中的每一项和 summary 必须使用简体中文描述；referenceText 和 generatedText 只记录图片中实际识别到的原始字符，必须保持原文、大小写、空格和标点，不得翻译或改写。` },
