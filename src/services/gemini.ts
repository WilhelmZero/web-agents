...
export function buildLogoReplacementInstruction(options: {
  ...
  return `执行严格的 Logo 替换任务。第一张图片是原始场景图，${referenceInstruction}${immutableSceneInstruction}${placementLockInstruction}${colorInstruction}${effectInstruction}${microTextInstruction}${exactTextInstruction}${correctionInstruction}${surfaceConformanceInstruction}${stemwareInstruction}${cupBottomInstruction}${antiOverlayInstruction}${zeroColorFinalCheck} 只允许改变旧 Logo 定位遮罩覆盖的区域：保持每个 Logo 原有中心点、上下高度、外包围区域、遮挡关系和材质融合方式，严禁把 Logo 自动移到杯子或载体中间。只有高脚杯专项或杯底安全区明确指出原 Logo 已越界时才允许在对应安全区内最小幅度修正。角度、透视和曲面包裹必须依据原定位处的载体真实几何重新计算，不能照抄可能错误的旧 Logo 形变，并用新 Logo 准确替换。若同一场景存在多个旧 Logo，必须全部替换。除 Logo 外，原图所有像素对应内容必须保持不变，包括画幅、构图、裁切、镜头、主体、产品结构、杯体、背景、人物、道具、已有非 Logo 文字、颜色、光线、阴影、反射、折射、景深、噪点和清晰度。不得移动、删除、增加、重绘或重新设计任何非 Logo 内容，不得在原本没有 Logo 的位置新增 Logo。`;
}
