import type {
  GeneratedImage,
  GlassLogoEtchOptions,
  ImageModel,
  ImageSize,
  LogoRemovalAnalysis,
  LogoRemovalVerification,
  LogoVerificationResult,
  ObjectPreservationOptions,
  OptimizerModel,
  SceneLogoStyle,
} from "../types";
...
import {
  ...
  type PaperTextRegion,
  type PaperTextVerification,
} from "./paperText";
import {
  geminiCapacityWaitMs,
  getGeminiCapacitySettings,
...
export async function testProxyConnection(
  ...
  try {
    ...
    throw new Error(
      "代理地址格式不正确，请填写完整的 http:// 或 https:// 地址",
    );
  }
...
function friendlyError(status: number, body: GeminiResponse): Error {
  ...
  if (status === 401 || status === 403)
    return new Error("API Key 无效、无权限或当前模型不可用");
  if (status === 429)
    return new Error("请求过于频繁或额度不足，请降低并发后重试");
  if (status === 503)
    return new Error(`Gemini 当前请求量过高，服务暂时无可用容量：${raw}`);
  if (status === 524)
    return new Error(
...
async function postGemini(
  ...
  const debugParts = (() => {
    ...
    const visit = (value: unknown) => {
      ...
      const inline = item.inlineData as
      ...
      if (inline?.data && images.length < 4) {
        try {
          images.push(
            new Blob(
              [
                Uint8Array.from(atob(inline.data), (char) =>
                  char.charCodeAt(0),
                ),
              ],
          ...
          /* Debug rendering must never block the request. */
        }
      }
  ...
  const consoleId = startRequestConsoleEntry({
    model,
    connection: apiBaseUrl ? "proxy" : "direct",
    requestSummary: summarizeGeminiRequest(body),
    ...
    try {
      ...
      if (sharedWait > 0) {
        updateRequestConsoleEntry(consoleId, {
          status: "retrying",
          attempt: attempt + 1,
          message: `Gemini 容量保护已触发，所有标签共享等待 ${Math.ceil(sharedWait / 1000)} 秒`,
        });
      ...
      if (response.ok && !data.error) {
        ...
        const outputImages = imageParts
          .slice(0, 4)
          .map(
            (part) =>
              new Blob(
                [
                  Uint8Array.from(atob(part.inlineData!.data), (char) =>
                    char.charCodeAt(0),
                  ),
                ],
        ...
        updateRequestConsoleEntry(consoleId, {
          ...
          resultSummary:
            (imageParts.length ? imageParts.length + " 张图片" : "响应成功") +
            (data.usageMetadata?.totalTokenCount
              ? " · " + data.usageMetadata.totalTokenCount + " tokens"
              : ""),
          message: "Gemini 请求完成",
          outputImages,
        });
      ...
      const capacityError = isGeminiCapacityError(
        response.status,
        data.error?.message,
      );
      ...
      if (!retryable || attempt >= retryLimit) {
        ...
        if (retryable)
          error.message += `；已自动重试 ${retryLimit} 次，建议稍后再试、降低并发或临时切换模型`;
        updateRequestConsoleEntry(consoleId, {
          status: "failed",
          httpStatus: response.status,
          durationMs: Math.round(performance.now() - requestStartedAt),
      ...
      updateRequestConsoleEntry(consoleId, {
        ...
        message: capacityError
          ? `Gemini 容量不足，已暂停所有标签 ${Math.ceil(delay / 1000)} 秒`
          : "HTTP " + response.status + "，等待自动重试",
      });
...
export async function generateSceneImage(options: {
      ...
      contents: [
        {
          ...
          parts: [
            {
              text: appendImageGenerationGuard(
                `基于提供的产品白底图生成一张商业场景图。必须保持产品外观、结构、颜色、Logo 和文字准确，不要复制或增加产品。场景要求：${options.prompt}`,
              ),
            },
  ...
  if (!imagePart?.inlineData?.data)
    throw new Error("模型未返回图片，请调整提示词后重试");

  const bytes = Uint8Array.from(atob(imagePart.inlineData.data), (char) =>
    char.charCodeAt(0),
  );
...
export async function generateSceneReplacementImage(options: {
      ...
      contents: [
        {
          ...
          parts: [
            {
              text: options.exactPrompt
                ? options.prompt
                : appendImageGenerationGuard(options.prompt),
            },
            { inlineData: { mimeType: options.image.type, data: base64 } },
          ],
        },
  ...
  const imagePart = data.candidates
    ...
    .find((part) => part.inlineData?.data);
  if (!imagePart?.inlineData?.data)
    throw new Error("模型未返回场景替换图片，请调整提示词后重试");
  const bytes = Uint8Array.from(atob(imagePart.inlineData.data), (char) =>
    char.charCodeAt(0),
  );
  const mimeType = imagePart.inlineData.mimeType || "image/png";
...
export async function optimizeSceneReplacePrompt(options: {
  apiKey: string;
  model: OptimizerModel;
  prompt: string;
  signal?: AbortSignal;
...
export async function generateLogoComposite(options: {
  ...
  const [sceneData, logoData, guideData] = await Promise.all([
    ...
    options.placementGuide
      ? fileToBase64(options.placementGuide)
  ...
  const parts: GeminiPart[] = [
    {
      text: appendImageGenerationGuard(
        `请完成专业的 Logo 场景合成。第一张图片是必须保持构图和内容的原始场景，第二张图片是必须准确保留图形、颜色和文字的原始 Logo。${placementInstruction}${applicationInstruction} 让 Logo 与场景的材质、透视、光线、阴影自然融合，不要在杯子以外的位置添加额外 Logo，不要改变场景中的主体。${options.prompt.trim() ? `用户补充要求：${options.prompt.trim()}` : ""}`,
      ),
    },
  ...
  if (guideData) {
    parts.push({
      inlineData: {
        mimeType: options.placementGuide?.type || "image/png",
        data: guideData,
      },
    });
  }
  ...
  const imagePart = data.candidates
    ...
    .find((part) => part.inlineData?.data);
  if (!imagePart?.inlineData?.data)
    throw new Error("模型未返回合成图片，请调整提示词后重试");
  const bytes = Uint8Array.from(atob(imagePart.inlineData.data), (char) =>
    char.charCodeAt(0),
  );
  const mimeType = imagePart.inlineData.mimeType || "image/png";
...
export function buildLogoReplacementInstruction(options: {
  ...
  const woodColorDepth = Math.max(
    0,
    Math.min(100, options.woodEngravingColorDepth ?? 15),
  );
  const woodColorProfile =
    woodColorDepth === 0
  ...
  const naturalRecessedInstruction =
    "原木同色浅雕或凹刻：颜色深浅参数为 " +
    woodColorDepth +
    "%。" +
    woodColorProfile +
    " 此百分比必须直接控制最终 Logo 相对当前木材底色的着色程度，不能使用固定默认色，也不能忽略或自行重置该参数。凹槽物理深度始终保持极浅且固定，颜色参数不得转化为更深的凹槽。保留 Logo 内外连续一致的真实木纹；Logo 参考图的黑色或白色仅代表图形蒙版和笔画形状，绝不能作为输出颜色。";
  const darkBurnInstruction =
  ...
  const woodMethod =
    woodStyle === "auto"
      ? "自动识别木盒底色并逐个选择工艺：若木盒为深色木材或深色涂层，使用深色激光烧蚀效果；若木盒为浅色木材或浅色涂层，使用原木同色浅雕或凹刻效果。不得把浅色木盒误用深黑烧蚀，也不得把深色木盒生成白色 Logo。浅色木盒采用以下浅雕颜色规则：" +
      ...
      : woodStyle === "dark-burn"
        ...
        : woodStyle === "natural-recessed"
          ? naturalRecessedInstruction
          : "自定义木盒雕刻方式：" +
            (options.customWoodEngravingMethod?.trim() ||
              "根据用户描述自然雕刻并保留木材纹理") +
            "。";
  const engravingInstructions = [
    options.glassEngravingEnabled
      ...
      : "",
    options.woodEngravingEnabled
      ? "若旧 Logo 位于木盒上，将新 Logo 按以下木盒工艺雕刻：" +
        woodMethod +
        " 必须严格服从上述工艺及颜色参数，再根据木盒实际木种、纹理方向和光照做自然材质融合；不得擅自加深颜色或凹槽，不得改变木盒本身的颜色、木纹、结构和构图。"
      : "",
    options.customEngravingEnabled
      ? "若旧 Logo 位于场景中的“" +
        customObject +
        "”上，采用指定雕刻方式：" +
        (customMethod || "根据载体材质自然雕刻") +
        "。必须服从该载体的颜色、纹理、硬度、反光和凹凸特性。"
      : "",
  ].filter(Boolean);
  const effectInstruction = automaticEngraving
    ...
    : engravingInstructions.length
      ? engravingInstructions.join("") +
        "必须先识别每个旧 Logo 所在的载体类型，再分别应用匹配的玻璃、木盒或自定义物体工艺；这些工艺可在同一张场景图中同时生效。未匹配上述载体的 Logo 保持原有制作工艺。"
      : "保持原 Logo 在场景中的现有制作工艺和材质融合方式，只替换 Logo 内容。";
  const microTextInstruction =
    "新 Logo 必须作为完整的不可拆分图形资产进行像素级外观复制，尤其是尺寸很小的文字、字母、数字、标点、细线和负空间。严禁对 Logo 执行 OCR 后重新输入、拼写、翻译、纠错、补字、猜字、改写字体或生成相似字母；不得把任何字符替换为其他字符，不得产生乱码。必须保持参考 Logo 中每个字符的数量、顺序、大小写、字形轮廓、间距、基线、粗细和相对位置完全一致。即使小字无法语义识别，也必须把原始笔画当作图形纹理逐笔保留，而不是解释其文字含义。";
  const exactTextInstruction = options.expectedText?.trim()
    ? "【准确文字最高优先级】新 Logo 中必须准确呈现以下字符序列：〈" +
      options.expectedText +
      "〉。必须逐字符保持内容、顺序、大小写、空格、数字和标点完全一致，禁止翻译、纠错、补全或替换。"
    : "";
  const correctionInstruction = options.correctionFeedback?.trim()
    ? "【上次结果校验失败，必须修复】" +
      options.correctionFeedback +
      "。针对校验指出的字符、旧 Logo 残留、位置尺寸、曲面贴合或材质融合问题逐项修复；不得用平面覆盖来规避问题，也不得改变场景其他内容。"
    : "";
  ...
  const zeroColorFinalCheck =
    !automaticEngraving &&
    options.woodEngravingEnabled &&
    options.woodEngravingStyle === "natural-recessed" &&
    woodColorDepth === 0
      ...
      : "";
  const stemwareInstruction =
    "【高脚杯专项】若载体是高脚杯、葡萄酒杯、香槟杯或鸡尾酒杯，先分割杯肚、杯梗和底座，杯梗与底座永远禁止放置 Logo。Logo 仅可位于杯肚正面上半部；其可见宽度不得超过杯肚最大可见宽度的 42%，中心应位于杯口下缘至杯肚底边有效高度的 32% 至 46%，底边不得低于该有效高度的 62%。若旧 Logo 本身过大或偏下，不得继承其错误尺寸和位置，必须将新 Logo 等比缩小并上移到上述安全区。宁可小而自然，也不得覆盖杯肚大面积、进入收窄区、靠近杯梗或底座。";
  const ribbedBeerMugInstruction = options.beerMugContext
    ? "【啤酒杯专项补充】仅因本图已识别为啤酒杯而启用：若杯身上部为连续光滑区域、下部为竖纹、棱柱、浮雕、切面或凹凸花纹区域，先检测下部纹理开始线。新 Logo 必须保持参考图原始宽高比并等比收纳在旧 Logo 所在的上部光滑区域，宁可缩小并留白，也不得裁切、拉伸、向下进入纹理区或移动到杯身中下部。旧 Logo 若贴近泡沫线或液面线，新 Logo 必须保持相同高度关系。此补充不得应用于非啤酒杯载体。"
    : "";
  const antiOverlayInstruction =
    "【禁止直接覆盖】必须先完整清除旧 Logo 的图形、文字、颜色和边缘，再按载体真实制作工艺重建新 Logo。严禁把新 Logo 原图作为矩形贴纸、平面图层、水印或不透明蒙版直接盖在原场景上；严禁保留参考图背景、方形边界、统一亮度或与载体无关的锐利边缘。新 Logo 内部必须连续继承载体原有的高光、阴影、透明度、反射、折射、纹理、颗粒、景深与遮挡，任何缺少这些变化的结果都视为不合格。";
  const immutableSceneInstruction =
    "【除旧 Logo 外整图冻结，最高优先级】先锁定旧 Logo 的每一个原始像素区域，只允许在这些区域内清除旧 Logo 并重建新 Logo；区域外的场景必须与原图完全一致。杯子、瓶子、礼盒、木盒、盒盖、内衬、人物、手和所有道具的外形、数量、大小、角度、朝向、位置、透视、裁切、遮挡、接触和承托关系都不得改变。原图中平放、侧放、倾斜或嵌在盒内内衬中的杯子必须保持完全相同的姿态和接触位置，严禁立起、悬浮、漂移、旋转、重新摆放或离开凹槽。新 Logo 必须继承旧 Logo 的前后层级和遮挡蒙版：若旧 Logo 被手、手指、杯沿、反光或其他物体挡住，新 Logo 的相同部分也必须位于遮挡物后方，严禁穿透遮挡或贴到手、手指、皮肤及前景物体表面。只替换原图实际存在的旧 Logo；任何原本没有旧 Logo 的表面都必须保持无 Logo，尤其禁止在盒子外侧、盒盖、盒子内部、内衬、锁扣、铰链及空白杯面上自行新增、复制或延伸 Logo。";
  return `执行严格的 Logo 替换任务。第一张图片是原始场景图，${referenceInstruction}${immutableSceneInstruction}${colorInstruction}${effectInstruction}${microTextInstruction}${exactTextInstruction}${correctionInstruction}${surfaceConformanceInstruction}${stemwareInstruction}${ribbedBeerMugInstruction}${antiOverlayInstruction}${zeroColorFinalCheck} 只允许改变旧 Logo 覆盖的区域：保持每个 Logo 原有的合理中心位置、大致覆盖范围、遮挡关系和材质融合方式；若旧 Logo 的位置、尺寸或融合本身明显错误，则必须按载体几何和上述安全区修正。角度、透视和曲面包裹必须依据载体真实几何重新计算，不能照抄可能错误的旧 Logo 形变，并用新 Logo 准确替换。若同一场景存在多个旧 Logo，必须全部替换。除 Logo 外，原图所有像素对应内容必须保持不变，包括画幅、构图、裁切、镜头、主体、产品结构、杯体、背景、人物、道具、已有非 Logo 文字、颜色、光线、阴影、反射、折射、景深、噪点和清晰度。不得移动、删除、增加、重绘或重新设计任何非 Logo 内容，不得在原本没有 Logo 的位置新增 Logo。`;
}

async function normalizedLogoReference(
  file: File,
    ...
    const blob = await new Promise<Blob>((resolve, reject) =>
      canvas.toBlob(
        (value) =>
          value
            ? resolve(value)
            : reject(new Error("Logo normalization failed")),
        "image/png",
      ),
    );
...
export async function generateLogoReplacement(options: {
      ...
      text: appendImageGenerationGuard(
        options.promptOverride?.trim() ||
          buildLogoReplacementInstruction({
            hasOldLogo: Boolean(options.oldLogo),
            logoColorMode: options.logoColorMode,
            customLogoColor: options.customLogoColor,
          }),
      ),
    },
    { inlineData: { mimeType: options.scene.type, data: sceneData } },
  ];
  if (oldLogoData && options.oldLogo)
    parts.push({
      inlineData: { mimeType: options.oldLogo.type, data: oldLogoData },
    });
  parts.push({
    inlineData: {
      mimeType: normalizedNewLogo.mimeType,
      data: normalizedNewLogo.data,
    },
  });
  const imageConfig: { imageSize: ImageSize; aspectRatio?: string } = {
    imageSize: options.imageSize,
  ...
  const imagePart = data.candidates
    ...
    .find((part) => part.inlineData?.data);
  if (!imagePart?.inlineData?.data)
    throw new Error("模型未返回 Logo 替换图片，请重试或切换模型");
  const bytes = Uint8Array.from(atob(imagePart.inlineData.data), (char) =>
    char.charCodeAt(0),
  );
  const mimeType = imagePart.inlineData.mimeType || "image/png";
...
export async function generateExactLogoReplacement(options: {
  apiKey: string;
  model: ImageModel;
  scene: File;
  oldLogo?: File;
  ...
  const [sceneData, oldLogoData, ...logos] = await Promise.all([
    ...
    options.oldLogo
      ? fileToBase64(options.oldLogo)
      : Promise.resolve(undefined),
    ...options.logos.map(normalizedLogoReference),
  ]);
  const parts: GeminiPart[] = [
    { text: options.prompt },
    { inlineData: { mimeType: options.scene.type, data: sceneData } },
  ...
  const imagePart = data.candidates
    ...
    .find((part) => part.inlineData?.data);
  if (!imagePart?.inlineData?.data)
    throw new Error("模型未返回自动分类 Logo 替换图片，请重试或切换模型");
  const bytes = Uint8Array.from(atob(imagePart.inlineData.data), (char) =>
    char.charCodeAt(0),
  );
  const mimeType = imagePart.inlineData.mimeType || "image/png";
...
export async function analyzeSceneLogoStyles(options: {
  ...
  const styles = (parsed.styles || []).map((style, index) => ({
    ...style,
    id: style.id || `style-${index + 1}`,
    occurrences: Math.max(1, Number(style.occurrences) || 1),
  }));
...
export async function generateMultiLogoReplacement(options: {
  ...
  const [sceneData, ...logoData] = await Promise.all([
    fileToBase64(options.scene),
    ...options.logos.map(fileToBase64),
  ]);
  const mapping = options.styles
    .map(
      (style, index) =>
        `原场景${style.label}（特征：${style.description}；载体：${style.carrier}；共 ${style.occurrences} 个位置）必须全部替换为第 ${index + 2} 张图片的新 Logo`,
    )
    .join("；");
  const parts: GeminiPart[] = [
    {
      text: appendImageGenerationGuard(
        `执行多样式 Logo 一对一替换。第一张图是必须保持不变的原场景。后续每张图分别是不同的新 Logo，禁止混用。映射关系：${mapping}。同一样式的所有位置必须使用映射到的同一个新 Logo，不同样式必须使用各自不同的新 Logo；不得遗漏、串换或在无 Logo 处新增。只允许修改每个旧 Logo 原本占据的区域，其他区域逐像素保持原图不变。杯子在盒内的平放、侧放、倾斜、内衬承托、位置和遮挡必须完全不变，严禁杯子立起、悬浮、移动或旋转。旧 Logo 若被手或手指遮挡，新 Logo 必须保持在手后方，严禁贴到手、皮肤或前景遮挡物上。盒子外侧、盒盖、盒内、内衬、锁扣、铰链等原本没有旧 Logo 的位置绝对禁止新增 Logo。必须先清除每处旧 Logo，再按该位置原有载体、曲率、透视、雕刻/印刷工艺、反射、折射、纹理、光线、景深和颗粒自然重建新 Logo，严禁平面贴图、水印或直接覆盖。除 Logo 区域外不得改变场景任何内容。${options.instruction}`,
      ),
    },
    { inlineData: { mimeType: options.scene.type, data: sceneData } },
    ...options.logos.map((logo, index) => ({
      inlineData: { mimeType: logo.type, data: logoData[index] },
    })),
  ];
  const data = await postGemini(
    options.model,
  ...
  const imagePart = data.candidates
    ...
    .find((part) => part.inlineData?.data);
  if (!imagePart?.inlineData?.data)
    throw new Error("模型未返回多 Logo 替换图片");
  const bytes = Uint8Array.from(atob(imagePart.inlineData.data), (char) =>
    char.charCodeAt(0),
  );
  const mimeType = imagePart.inlineData.mimeType || "image/png";
...
export async function verifyLogoReplacement(options: {
  ...
  const [reference, original, generated] = await Promise.all([
    normalizedLogoReference(options.referenceLogo),
    fileToBase64(options.originalScene),
    fileToBase64(options.generatedImage),
  ]);
      ...
      contents: [
        {
          ...
          parts: [
            {
              text: `第一张图片是新 Logo 参考，第二张图片是替换前的原始场景，第三张图片是生成后的场景。执行严格的 Logo 替换验收，以下各项必须全部通过才允许 passed=true：一、新 Logo 的字符、图形和比例与参考一致；二、原 Logo 已完整移除，没有新旧 Logo 重叠或残影；三、新 Logo 真实贴合原 Logo 所在载体的曲面、透视、折射、反射、纹理、凹凸、环境光、景深和颗粒；四、新 Logo 的位置和尺寸处于原 Logo 所在载体的合理视觉区域；五、除原 Logo 区域外，杯子、盒子、内衬、人物、手、背景和道具的形状、位置、角度、姿态、遮挡、接触及承托关系与原图完全一致；六、没有在任何原本无旧 Logo 的位置新增 Logo。重点检查杯子原本平放或嵌在盒内却变成立起、悬浮、移动、旋转或离开内衬的情况；出现任一情况必须 placementConsistent=false、passed=false。若旧 Logo 原本被手或手指挡住，新 Logo 必须仍在手后方；Logo 穿过手、覆盖手或贴在皮肤上时必须 materialIntegrated=false、placementConsistent=false、passed=false。盒子外侧、盒盖、盒子内部、内衬、锁扣或铰链原本没有旧 Logo 却出现新 Logo 时必须 placementConsistent=false、passed=false，并在 differences 中明确指出。${options.beerMugContext ? "本图已识别为啤酒杯：若上部光滑、下部有竖纹或浮雕，新 Logo 必须等比缩小并完整保留在旧 Logo 所在的上部光滑区，不得进入纹理区；若旧 Logo 靠近泡沫线或液面线，必须保持相同高度关系。此规则只用于本张啤酒杯图片。" : ""}${expected ? `准确文字必须逐字符等于〈${expected}〉，包括大小写、空格、数字和标点。` : "未提供准确文字，请直接比较参考 Logo 与生成 Logo 的全部可见字符、笔画、布局和图形。"}重点识别“直接覆盖”：如果新 Logo 像平面贴纸、水印或图层一样叠在场景上，必须设置 flatOverlayDetected=true、materialIntegrated=false、passed=false。对于高脚杯、葡萄酒杯、香槟杯和鸡尾酒杯，Logo 必须位于杯肚上半部，宽度不超过杯肚最大宽度约 42%，不得接近杯梗或底座；过大或偏下必须 placementConsistent=false、passed=false。任何字符错误、旧 Logo 残留、非 Logo 内容变化、新增 Logo、遮挡错误、平面覆盖、材质未融合或位置尺寸异常都判定失败。differences 数组中的每一项和 summary 必须使用简体中文描述；referenceText 和 generatedText 只记录图片中实际识别到的原始字符，必须保持原文、大小写、空格和标点，不得翻译或改写。`,
            },
            {
              inlineData: {
            ...
            {
              inlineData: {
                mimeType: options.originalScene.type,
                data: original,
              },
            },
            {
              inlineData: {
                mimeType: options.generatedImage.type || "image/png",
                data: generated,
              },
            },
          ],
      ...
      generationConfig: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            graphicConsistent: { type: "BOOLEAN" },
            materialIntegrated: { type: "BOOLEAN" },
            placementConsistent: { type: "BOOLEAN" },
            originalLogoRemoved: { type: "BOOLEAN" },
            flatOverlayDetected: { type: "BOOLEAN" },
            summary: { type: "STRING" },
          },
          required: [
            "passed",
  ...
  try {
    ...
    if (
      ...
      typeof parsed.graphicConsistent !== "boolean" ||
      typeof parsed.materialIntegrated !== "boolean" ||
      typeof parsed.placementConsistent !== "boolean" ||
      typeof parsed.originalLogoRemoved !== "boolean" ||
      typeof parsed.flatOverlayDetected !== "boolean"
    )
      throw new Error("invalid");
    const passed =
      parsed.passed &&
      parsed.graphicConsistent &&
      parsed.materialIntegrated &&
      parsed.placementConsistent &&
      parsed.originalLogoRemoved &&
      !parsed.flatOverlayDetected;
    return {
      ...parsed,
      passed,
      referenceText: String(parsed.referenceText || ""),
      generatedText: String(parsed.generatedText || ""),
      differences: parsed.differences.map(String),
...
export async function analyzeLogoRemovalGemini(options: {
  apiKey: string;
  model: OptimizerModel;
  scene: File;
  prompt: string;
    ...
    options.apiKey,
    {
      ...
      generationConfig: {
        ...
        responseSchema: {
          type: "OBJECT",
          properties: {
            action: { type: "STRING", enum: ["remove", "skip_no_target"] },
            summary: { type: "STRING" },
            reason: { type: "STRING" },
            targets: {
              type: "ARRAY",
  ...
  return {
    ...parsed,
    action:
      parsed.action === "remove" && parsed.targets.length
        ? "remove"
        : "skip_no_target",
  };
}

export async function generateLogoRemovalGemini(options: {
  apiKey: string;
  model: ImageModel;
  scene: File;
  prompt: string;
  ...
  const imagePart = data.candidates
    ...
    .find((part) => part.inlineData?.data);
  if (!imagePart?.inlineData?.data)
    throw new Error("模型未返回去除 Logo 后的图片");
  const bytes = Uint8Array.from(atob(imagePart.inlineData.data), (char) =>
    char.charCodeAt(0),
  );
  const mimeType = imagePart.inlineData.mimeType || "image/png";
...
export async function verifyLogoRemovalGemini(options: {
  apiKey: string;
  model: OptimizerModel;
  originalScene: File;
  generatedImage: Blob;
  ...
  signal?: AbortSignal;
  apiBaseUrl?: string | null;
}): Promise<LogoRemovalVerification> {
    ...
    options.apiKey,
    {
      ...
      generationConfig: {
        ...
        responseSchema: {
          ...
          properties: {
            passed: { type: "BOOLEAN" },
            logoRemoved: { type: "BOOLEAN" },
            reconstructionNatural: { type: "BOOLEAN" },
            nonTargetPreserved: { type: "BOOLEAN" },
            differences: { type: "ARRAY", items: { type: "STRING" } },
            summary: { type: "STRING" },
          },
          required: [
  ...
  return {
    ...
    passed: Boolean(
      parsed.passed &&
      parsed.logoRemoved &&
      parsed.reconstructionNatural &&
      parsed.nonTargetPreserved,
    ),
  };
}
export function buildObjectReplacementInstruction(options: {
  ...
  const ordinal = (index: number) =>
    ["第一", "第二", "第三"][index - 1] || "第" + index;
  const references: string[] = [
    "第一张图片是原始场景图，也是最终画面、构图、机位、光照、遮挡和环境的唯一基准。",
  ];
  if (options.hasSourceReference)
    references.push(
      `${ordinal(imageIndex++)}张图片仅用于帮助识别原物体“${sourceName}”，不得复制该参考图的背景或外观到结果。`,
    );
  if (options.hasTargetReference)
    references.push(
      `${ordinal(imageIndex)}张图片是新物体本体参考图。只提取“${targetName}”实体本身的核心身份特征，不得复制参考图中的背景、构图、相机角度、摆放姿态、阴影、反射、手、道具或附属内容。`,
    );
...
export async function generateObjectReplacementImage(options: {
  ...
  const [sceneData, sourceData, targetData] = await Promise.all([
    ...
    options.sourceReference
      ? fileToBase64(options.sourceReference)
      : Promise.resolve(undefined),
    options.targetReference
      ? fileToBase64(options.targetReference)
      : Promise.resolve(undefined),
  ]);
  const parts: GeminiPart[] = [
    {
      text: appendImageGenerationGuard(
        buildObjectReplacementInstruction({
  ...
  if (sourceData && options.sourceReference)
    parts.push({
      inlineData: { mimeType: options.sourceReference.type, data: sourceData },
    });
  if (targetData && options.targetReference)
    parts.push({
      inlineData: { mimeType: options.targetReference.type, data: targetData },
    });
  const imageConfig: { imageSize: ImageSize; aspectRatio?: string } = {
    imageSize: options.imageSize,
  };
  ...
  const imagePart = data.candidates
    ...
    .find((part) => part.inlineData?.data);
  if (!imagePart?.inlineData?.data)
    throw new Error("模型未返回物体替换图片，请重试或切换模型");
  const bytes = Uint8Array.from(atob(imagePart.inlineData.data), (char) =>
    char.charCodeAt(0),
  );
  const mimeType = imagePart.inlineData.mimeType || "image/png";
...
export async function generateInpaintImage(options: {
      ...
      contents: [
        {
          ...
          parts: [
            {
              text: appendImageGenerationGuard(
                `请执行严格的局部重绘。第一张图片是必须保留的原始图片；第二张图片是区域参考图，其中红色半透明标记是唯一允许修改的区域。只在红色标记区域内根据用户要求生成或修改内容，标记区域外的所有像素对应内容必须保持不变，包括主体、构图、机位、裁切、背景、光线、阴影、颜色、材质、文字和 Logo。不得扩大修改区域，最终结果不得出现红色遮罩、选区边缘或标记。用户要求：${options.prompt}`,
              ),
            },
            { inlineData: { mimeType: options.image.type, data: imageData } },
            {
              inlineData: {
                mimeType: options.maskGuide.type || "image/png",
                data: maskData,
              },
            },
          ],
  ...
  const imagePart = data.candidates
    ...
    .find((part) => part.inlineData?.data);
  if (!imagePart?.inlineData?.data)
    throw new Error("模型未返回局部重绘图片，请调整选区或提示词后重试");
  const bytes = Uint8Array.from(atob(imagePart.inlineData.data), (char) =>
    char.charCodeAt(0),
  );
  const mimeType = imagePart.inlineData.mimeType || "image/png";
...
export async function generateCupResizeImage(options: {
      ...
      contents: [
        {
          ...
          parts: [
            {
              text: appendImageGenerationGuard(
                `${options.prompt}${options.supplementalPrompt?.trim() ? `\n用户补充要求：${options.supplementalPrompt.trim()}` : ""}`,
              ),
            },
            {
              inlineData: {
                mimeType: options.compositeGuide.type || "image/png",
                data: guideData,
              },
            },
          ],
  ...
  const imagePart = data.candidates
    ...
    .find((part) => part.inlineData?.data);
  if (!imagePart?.inlineData?.data)
    throw new Error("模型未返回杯子尺寸调整图片，请重试或切换模型");
  const bytes = Uint8Array.from(atob(imagePart.inlineData.data), (char) =>
    char.charCodeAt(0),
  );
  const mimeType = imagePart.inlineData.mimeType || "image/png";
...
export async function recognizePaperTextGemini(options: {
  apiKey: string;
  model: string;
  image: File;
  signal?: AbortSignal;
  apiBaseUrl?: string | null;
}): Promise<PaperTextRegion[]> {
  ...
  try {
    return normalizePaperTextRegions(JSON.parse(raw));
  } catch {
    throw new Error("Gemini 文字识别结果格式无效");
  }
}

export async function editPaperTextGemini(options: {
  apiKey: string;
  model: string;
  image: File;
  prompt: string;
...
export async function verifyPaperTextGemini(options: {
  apiKey: string;
  model: string;
  image: Blob;
  regions: PaperTextRegion[];
  signal?: AbortSignal;
  apiBaseUrl?: string | null;
}): Promise<PaperTextVerification> {
  const imageData = await fileToBase64(options.image);
  const targets = options.regions
    .filter((r) => r.text !== r.original)
    .map((r) => `“${r.original}”→“${r.text}”`)
    .join("\n");
  const data = await postGemini(
    options.model,
    options.apiKey,
      ...
      contents: [
        {
          ...
          parts: [
            {
              text: `严格检查图片中的花纸文字修改：\n${targets}\n只有全部新文字准确、旧文字消失、非目标区域没有变化时 ok 才能为 true。reason 使用简体中文。`,
            },
            {
              inlineData: {
  ...
  try {
    return JSON.parse(raw) as PaperTextVerification;
  } catch {
    throw new Error("Gemini 复核结果格式无效");
  }
}
...
用户商品信息：${options.productInfo}`,
  ...
  if (
    ...
    parsed.prompts.length !== options.count
  ) {
    throw new Error(
      `商品分析应返回 ${options.count} 条提示词，实际返回 ${parsed.prompts?.length || 0} 条`,
    );
  }
  ...
  return parsed.prompts.map((item, index) => {
    if (
      ...
      !item.content.trim() ||
      !Array.isArray(item.overlayTexts) ||
      !item.overlayTexts.every((value) => typeof value === "string")
    ) {
    ...
    return {
      ...
      overlayTexts: item.overlayTexts
...
export async function generateProductDetailImage(options: {
      ...
      contents: [
        {
          ...
          parts: [
            {
              text: appendImageGenerationGuard(
                `请基于第一张白底产品图制作一张专业电商商品详情页图片。必须严格保持产品外观、结构、颜色、材质、真实比例、Logo 和已有文字准确，不得重新设计、复制或增加产品。按照下面的详情页提示完成场景、构图、卖点表达和指定上图文字；引号内文字需要清晰准确地显示。详情页要求：${options.prompt}`,
              ),
            },
            { inlineData: { mimeType: options.image.type, data: imageData } },
          ],
  ...
  const imagePart = data.candidates
    ...
    .find((part) => part.inlineData?.data);
  if (!imagePart?.inlineData?.data)
    throw new Error("模型未返回商品详情图，请调整提示词后重试");
  const bytes = Uint8Array.from(atob(imagePart.inlineData.data), (char) =>
    char.charCodeAt(0),
  );
  const mimeType = imagePart.inlineData.mimeType || "image/png";
