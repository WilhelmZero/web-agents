...
export interface BorderMatteAnalysis { matte: RgbColor; confidence: number; sampledPixels: number; isBlack: boolean }
...
export function detectBorderMatteFromPixels(data: Uint8ClampedArray, width: number, height: number): RgbColor {
  return analyzeBorderMatteFromPixels(data, width, height).matte;
}

export function analyzeBorderMatteFromPixels(data: Uint8ClampedArray, width: number, height: number): BorderMatteAnalysis {
  ...
  const matte = { r: Math.round(winner.r / winner.count), g: Math.round(winner.g / winner.count), b: Math.round(winner.b / winner.count) };
  return { matte, confidence: sampledPixels ? winner.count / sampledPixels : 0, sampledPixels, isBlack: Math.max(matte.r, matte.g, matte.b) <= 64 };
}

export async function detectBorderMatte(blob: Blob): Promise<RgbColor> {
  return (await analyzeBorderMatte(blob)).matte;
}

export async function analyzeBorderMatte(blob: Blob): Promise<BorderMatteAnalysis> {
  ...
  try {
    ...
    return analyzeBorderMatteFromPixels(context.getImageData(0, 0, canvas.width, canvas.height).data, canvas.width, canvas.height);
