...
export function buildLogoRemovalAnalysisPrompt(scopes: LogoRemovalScope | readonly LogoRemovalScope[], customScope = '') {
  ...
  return `你是商业产品图的 Logo 去除目标分析器。当前勾选范围的并集为：${scopeDescription}。只把真实附着在这些选定产品载体或区域表面的品牌 Logo、姓名图案、个性化印刷、雕刻、蚀刻、烙印或贴附标识识别为目标，未列入勾选范围的载体不属于目标。不得把商品说明、尺寸箭头和数字、容量文字、卖点图标、版式标题、背景招牌、场景装饰、杯中反射、木纹或玻璃纹理误判为目标。逐个输出目标在整图中的 0 到 1 归一化包围框，并说明载体、工艺和人物/手等遮挡关系。preserve 列出必须保留的商品说明、尺寸标注、人物、非目标区域的礼盒或木盒、背景文字及其他关键元素。没有符合当前范围的目标时 action 必须为 skip_no_target，不能为了处理而虚构 Logo。所有说明使用简体中文。`;
}
...
export async function analyzeLogoRemovalTarget(options: { settings: LogoRemovalSettings; apiKey: string; openAiApiKey: string; apiBaseUrl?: string | null; scene: File; signal?: AbortSignal }): Promise<LogoRemovalAnalysis> {
  const prompt = buildLogoRemovalAnalysisPrompt(options.settings.scopes?.length ? options.settings.scopes : options.settings.scope || 'cup-body', options.settings.customScope);
