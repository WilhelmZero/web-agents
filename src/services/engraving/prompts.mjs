...
export function buildPrompt({ subject = 'auto', instructions = '', style = 'strong', hasReference = false, feedback = '', editMode = false, outpaint } = {}) {
  ...
  let prompt = `Edit the supplied customer image into a production-oriented monochrome laser-engraving artwork for a dark-coated cup. Use the input image as the identity and geometry source. The final image will be composited onto PURE BLACK (#000000): black means no engraving and light marks mean engraving.
...
${feedback ? `\nCORRECTIONS FROM THE PREVIOUS QUALITY CHECK (still use image 1 as the only identity source)\n${feedback.slice(0, 3000)}` : ''}`;
  ...
  prompt += '\nMANDATORY SOURCE CHECK: The deliverable must depict the customer subjects, never the style-reference subjects. Do not return, trace, reconstruct, or choose Image 2 as the output. Before producing the image, verify the number of people/animals, faces, clothing and held objects against the customer photograph. The reference provides texture only, even when it already looks like a finished engraving.';
  if (!editMode && outpaint?.enabled) return prompt
    .replace('Image 1 is the ONLY source for identity, anatomy, clothing, objects, pose, composition and number of subjects.', 'Image 1 is the customer photograph placed on an expanded edit canvas, NOT an existing engraving candidate. Image 3 is the ONLY source for identity, anatomy, clothing, objects, pose, composition and number of subjects; it is the unmodified original customer photograph.')
    .replace("Transfer its engraving treatment onto Image 1 while preserving Image 1's exact subjects.", "Apply the engraving treatment to the customer on Image 1, using Image 3 to preserve the exact original subjects. Image 2 remains style only.")
    .replace('(still use image 1 as the only identity source)', '(use image 3 as the original identity source; image 1 is its expanded canvas; image 2 is style only)');
  if (!editMode) return prompt;
  return prompt
    .replace('Edit the supplied customer image', 'Make targeted improvements to the supplied existing engraving candidate')
    .replace('Use the input image as the identity and geometry source.', 'Image 1 is the existing engraving to edit. Preserve its successful details. Image 3 is the original photograph and the authoritative identity and geometry source.')
