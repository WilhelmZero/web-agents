...
function assessed(review, target, outpaint) {
  ...
  const passed = score >= target && review.integrity !== "mismatch";
  return { ...review, score, passed, issueLabels: review.issues.map(key => ISSUES[key]) };
}

const FIXES = {
  identity: 'Restore the exact original subject identity, visible geometry, expressions, anatomy and pose from the ORIGINAL_CUSTOMER_SOURCE. Do not beautify or change identities.',
...
export async function runAutoTune({ sourceProfile, original, reference, config, subject, instructions, style, outpaint, params: initialParams, options,
  ...
  try {
    for (let round = 1; round <= maxRounds; round++) {
      ...
      if (action === 'generate') {
        const base = continueOnGenerated && best?.integrity !== 'mismatch' ? best : null;
      ...
      if (cancelled()) return state('cancelled', '已停止，保留现有结果');
      checks++;
      const assessment = assessed(validateReview(await review({ sourceProfile, original, reference, rendered: rendered.buffer, config, params, instructions, outpaint })), targetScore, outpaint);
      ...
      const rank = item => (item.integrity === 'mismatch' ? -10000 : 0) + (item.passed ? 1000 : 0) + Math.min(item.scores.identity, item.scores.subjects) * 2 + item.score;
