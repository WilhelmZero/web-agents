...
每个结果的下载以及“下载所选”统一进入输出尺寸弹窗，支持毫米＋DPI或像素宽度/边距，并显示实际尺寸。默认 80mm、300 DPI、4mm；输出设置使用独立 `custom-monochrome-logo:export:v1`，不修改运行参数。确认时冻结所选原图与参数，单张 PNG、多张 ZIP；部分失败允许下载已成功部分，不重发 AI 请求。旧任务进入页面时兼容无裁剪、无手动覆盖的记录。
