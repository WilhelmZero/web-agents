...
- 页面复用全局 OpenAI/GPT 密钥与共享 GPT 地址（`openAiEndpoint.ts`），不再保存或读取旧的工具专用地址，也不混用 Gemini 代理。模型、质量、雕刻及输出参数集中在右侧独立滚动栏。`images/edits` 只按顺序发送原照和风格参考；`responses` 审核发送原照、同一风格参考和渲染结果。连接测试仅 GET `/models`。
...
每次 AI 返回图以独立结果保存：原始图片、初始及当前参数、实际生成提示词、当次风格参考及审核记录。展开单张结果可调参、擦除、查看历史和下载；操作不会修改其他图片或历史审核快照。旧任务进入页面时自动转换为逐张结果。
