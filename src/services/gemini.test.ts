...
describe("Gemini API 地址", () => {
  it("未配置代理时直连 Google", () => {
    expect(getGeminiApiRoot("")).toBe(
      "https://generativelanguage.googleapis.com/v1beta",
    );
  });

  it("为 Worker 根地址补充 v1beta 路径", () => {
    expect(getGeminiApiRoot("https://proxy.example.workers.dev/")).toBe(
      "https://proxy.example.workers.dev/v1beta",
    );
  });

  it("不会重复追加已有的 v1beta 路径", () => {
    expect(getGeminiApiRoot("https://proxy.example.workers.dev/v1beta")).toBe(
      "https://proxy.example.workers.dev/v1beta",
    );
  });

  it("从代理根地址或 v1beta 地址生成健康检查地址", () => {
    expect(getProxyHealthUrl("https://proxy.example.workers.dev/")).toBe(
      "https://proxy.example.workers.dev/health",
    );
    expect(getProxyHealthUrl("https://proxy.example.workers.dev/v1beta")).toBe(
      "https://proxy.example.workers.dev/health",
    );
  });
  ...
  it("只对临时性服务错误进行自动重试", () => {
    expect(
      [408, 429, 500, 502, 503, 504, 524].every(isRetryableGeminiStatus),
    ).toBe(true);
...
describe("Gemini 图片编辑真实性保护", () => {
  ...
  it("adds the anti-red-cast and realistic-skin guard to scene replacement requests", async () => {
          ...
          candidates: [
            {
              content: {
                parts: [
                  {
                    inlineData: { data: btoa("result"), mimeType: "image/png" },
                  },
                ],
              },
            },
    ...
    await generateSceneReplacementImage({
      ...
      prompt: "替换背景",
      image: new File(["scene"], "scene.png", { type: "image/png" }),
      imageSize: "2K",
      apiBaseUrl: null,
    });
  ...
  it("submits exact scene prompts without appending any guard text", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(
        JSON.stringify({
          candidates: [
    ...
    const exact = "只使用这段分类提示词，不追加任何内容。";
    await generateSceneReplacementImage({
      apiKey: "test-key",
      model: "gemini-3.1-flash-image",
      prompt: exact,
    ...
    const body = JSON.parse(String(fetchMock.mock.calls[0]?.[1]?.body));
    expect(body.contents[0].parts[0].text).toBe(exact);
  });

  it("submits exact Logo preset text with ordered reference images", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(
        JSON.stringify({
          candidates: [
    ...
    const exact = "只提交用户保存的 Logo 替换预设。";
...
describe("Logo 结果校验", () => {
  ...
  it("提交参考图、生成图和准确文字并解析结构化结果", async () => {
    ...
    const originalScene = new File(["scene"], "scene.png", {
      type: "image/png",
    });
    ...
    expect(body.contents[0].parts[0].text).toContain(
      "differences 数组中的每一项和 summary 必须使用简体中文描述",
    );
    expect(body.contents[0].parts[0].text).toContain(
      "referenceText 和 generatedText 只记录图片中实际识别到的原始字符",
    );
    expect(body.contents[0].parts[0].text).toContain(
      "flatOverlayDetected=true",
    );
    expect(body.contents[0].parts[0].text).toContain(
      "宽度不超过杯肚最大宽度约 42%",
    );
  ...
  it("模型误报通过时，平面覆盖或融合失败会被代码强制判为失败", async () => {
    ...
    const result = await verifyLogoReplacement({
      ...
      generatedImage: new Blob(["result"], { type: "image/png" }),
      apiBaseUrl: null,
    });
...
describe("Logo 替换指令", () => {
  ...
  it("区分木盒深色烧蚀并保护 Logo 小字字形", () => {
    const instruction = buildLogoReplacementInstruction({
      hasOldLogo: true,
      logoColorMode: "white",
      woodEngravingEnabled: true,
      woodEngravingStyle: "dark-burn",
    });
  ...
  it("支持木盒原木同色浅雕和自定义方式", () => {
    const natural = buildLogoReplacementInstruction({
      hasOldLogo: false,
      logoColorMode: "original",
      woodEngravingEnabled: true,
      woodEngravingStyle: "natural-recessed",
      woodEngravingColorDepth: 0,
    });
    ...
    expect(natural).toContain(
      "黑色、白色及任何颜色像素只能用于确定凹槽的形状和位置",
    );
    expect(natural).toContain("如果初步结果看起来像深色线稿");
    const darkNatural = buildLogoReplacementInstruction({
      hasOldLogo: false,
      logoColorMode: "original",
      woodEngravingEnabled: true,
      woodEngravingStyle: "natural-recessed",
      woodEngravingColorDepth: 85,
    });
    expect(darkNatural).toContain("22%–38%");
    expect(darkNatural).toContain("凹槽物理深度始终保持极浅且固定");
    const custom = buildLogoReplacementInstruction({
      hasOldLogo: false,
      logoColorMode: "original",
      woodEngravingEnabled: true,
      woodEngravingStyle: "custom",
      customWoodEngravingMethod: "浅金色精细线雕",
    });
    expect(custom).toContain("浅金色精细线雕");
  });

  it("玻璃默认使用激光磨砂雕刻并支持其他自定义载体", () => {
    const glass = buildLogoReplacementInstruction({
      hasOldLogo: false,
      logoColorMode: "original",
    ...
    const custom = buildLogoReplacementInstruction({
      hasOldLogo: false,
      logoColorMode: "original",
      customEngravingEnabled: true,
      customEngravingObject: "深蓝色皮革盒",
      engravingMethod: "低温压凹",
    });
...
describe("物体批量替换指令", () => {
  it("只移植新杯本体和勾选的表面特征，其他内容跟随原场景", () => {
    ...
    expect(instruction).toContain(
      "旧物体轮廓与按新物体真实比例放置后的新轮廓之并集",
    );
    expect(instruction).toContain(
      "旧物体上的同类印花、Logo、文字和雕刻全部属于旧物体本体",
    );
  ...
  it("未勾选目标参考元素时不从目标图带入并保持原场景内容", () => {
    ...
    expect(instruction).toContain(
      "不要强制复制新物体参考图上的印花、Logo、雕刻或装饰",
    );
    expect(instruction).toContain("原场景有液体时必须保留");
    expect(instruction).toContain("原场景没有时不得新增");
    expect(instruction).toContain(
      "原场景是除新物体本体身份之外所有信息的最高优先级来源",
    );
    expect(instruction).toContain(
      "变化必须仅限于旧物体轮廓、新物体真实比例轮廓及其紧邻融合边缘",
    );
  });
});
