...
export function generateMultiLogoReplacementOpenAi(options: { apiKey: string; model: 'gpt-image-2'; scene: File; logos: File[]; styles: SceneLogoStyle[]; instruction: string; signal?: AbortSignal }) {
  ...
  return editImages({ ...options, images: [options.scene, ...options.logos], prompt: `第一张图是必须保持不变的原始场景，后续图片是不同的新 Logo。映射：${mapping}。同一样式所有位置必须使用同一个对应 Logo，不得混用、遗漏或新增。只修改旧 Logo 区域，保持场景、构图、人物、产品、光影和材质不变；新 Logo 必须贴合载体曲率、透视、反射、折射和原有印刷或雕刻工艺，不得直接平面覆盖。${options.instruction}` });
}
...
export async function verifyLogoReplacementOpenAi(options: { apiKey: string; model: string; referenceLogo: File; originalScene: File; generatedImage: Blob; expectedText?: string; signal?: AbortSignal }): Promise<LogoVerificationResult> {
  ...
  const parsed = await requestJson({ ...options, images: [options.referenceLogo, options.originalScene, options.generatedImage], schemaName: 'logo_verification', schema: verificationSchema, prompt: `第一张是新 Logo 参考，第二张是替换前场景，第三张是生成结果。严格检查：新 Logo 图形文字和比例一致；旧 Logo 完整移除；材质、曲率、透视、光影真实融合；位置尺寸合理；不得是平面覆盖。${expected ? `准确文字必须逐字符等于〈${expected}〉。` : ''}所有说明使用简体中文。` }) as LogoVerificationResult;
