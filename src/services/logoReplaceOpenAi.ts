...
export async function verifyLogoReplacementOpenAi(options: { apiKey: string; model: string; referenceLogo: File; originalScene: File; generatedImage: Blob; expectedText?: string; beerMugContext?: boolean; signal?: AbortSignal }): Promise<LogoVerificationResult> {
  const expected = options.expectedText?.trim();
  const parsed = await requestJson({ ...options, images: [options.referenceLogo, options.originalScene, options.generatedImage], schemaName: 'logo_verification', schema: verificationSchema, prompt: `第一张是新 Logo 参考，第二张是替换前场景，第三张是生成结果。严格检查：新 Logo 图形文字和比例一致；旧 Logo 完整移除；材质、曲率、透视、光影真实融合；位置尺寸合理；不得是平面覆盖。除旧 Logo 区域外整图必须与原图一致：杯子、盒子、内衬、人物、手、背景和道具不得移动、旋转、重绘或改变承托遮挡关系。杯子原本平放或嵌在盒内却变成立起、悬浮、移动、旋转或离开内衬时，必须 placementConsistent=false、passed=false。旧 Logo 被手遮挡而新 Logo 覆盖手、穿过手或贴到皮肤上时，必须 materialIntegrated=false、placementConsistent=false、passed=false。盒子、盒盖、盒内、内衬、锁扣或铰链原本无旧 Logo 却新增 Logo 时，必须 placementConsistent=false、passed=false。${options.beerMugContext ? '本图已识别为啤酒杯：若上部光滑、下部有竖纹或浮雕，新 Logo 必须等比缩小并完整保留在旧 Logo 所在的上部光滑区，不得进入纹理区；若旧 Logo 靠近泡沫线或液面线，必须保持相同高度关系。此规则只用于本张啤酒杯图片。' : ''}${expected ? `准确文字必须逐字符等于〈${expected}〉。` : ''}所有说明使用简体中文。` }) as LogoVerificationResult;
