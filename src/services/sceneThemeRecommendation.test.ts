...
describe('scene theme recommendation', () => {
  ...
  it('contains every requested cup-to-lifestyle mapping and excludes festivals', () => { const prompt = buildSceneThemeRecommendationPrompt(); CUP_SCENE_RULES.forEach((rule) => expect(prompt).toContain(rule)); expect(prompt).toContain('严禁推荐圣诞节'); expect(prompt).toContain('真实杯型和主要用途'); });
