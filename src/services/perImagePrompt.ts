...
export function buildPerImageAnalysisPrompt(tool: PerImagePromptTool, sourcePrompt: string) {
  const focus = tool === 'logo-replace'
    ...
    : '识别杯型、人物及手势、礼盒/木盒、产品说明文字、尺寸标注、功能图标、悬浮营销标题、杯身 Logo/刻字、裁切主体、图中图和当前构图，并评估真正环境背景的可编辑面积。如果存在多个小图或场景面板，必须按阅读顺序逐格列出每格的原环境与应执行的目标变化，明确要求每一格都发生可见的环境替换，不能遗漏、轻微处理或保持任意一格不变。把所有不属于真实环境的商品信息叠加层视为受保护前景，不得当作背景删除；木盒、礼盒和包装无论占据多大面积都属于商品。若某一格真实背景面积很少，该格提示词必须改为仅在剩余背景、台面材质、光影和少量边缘装饰中表达主题，不得要求完整房间，但也不得跳过该格。识别所有原有产品和参照物之间的相对比例，禁止为适配新场景而缩放产品。在摘要和适用条件中按可见原文及位置列出需要保留的文字与图标。只保留与本图有关的条件。';
