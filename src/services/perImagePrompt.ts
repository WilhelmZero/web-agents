...
export function buildPerImageAnalysisPrompt(tool: PerImagePromptTool, sourcePrompt: string) {
  const focus = tool === 'logo-replace'
    ...
    : '识别杯型、人物及手势、礼盒、产品说明文字、尺寸标注、功能图标、悬浮营销标题、杯身 Logo/刻字、裁切主体、图中图和当前构图，并选择与本图真实杯型和用途相符的非节日欧美生活场景条件。把所有不属于真实环境的商品信息叠加层视为受保护前景，不得当作背景删除；在摘要和适用条件中按可见原文及位置列出需要保留的文字与图标。只保留与本图有关的条件。';
