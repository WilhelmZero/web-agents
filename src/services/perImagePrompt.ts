...
import { getSceneReplacementConstraintCatalog } from './sceneReplacementPrompt';
import { fileToBase64 } from '../utils';
import { startRequestConsoleEntry, updateRequestConsoleEntry } from './requestConsole';

export const LOGO_PER_IMAGE_ANALYSIS_VERSION = 3;
...
export function parsePerImagePromptResult(text: string) {
  ...
  const value = JSON.parse(match[0]) as { summary?: unknown; applicableConditions?: unknown; prompt?: unknown; constraints?: unknown; action?: unknown; actionReason?: unknown };
  ...
  const action: PerImagePromptAssignment['action'] = value.action === 'skip-no-logo' || value.action === 'skip-gift-scene' ? value.action : 'replace';
  const actionReason = typeof value.actionReason === 'string' ? value.actionReason.trim() : '';
  if (!summary || !prompt) throw new Error('语言模型返回的图片摘要或独立提示词为空');
  return { summary, applicableConditions, prompt, constraints, action, actionReason };
}
...
export function buildPerImageAnalysisPrompt(tool: PerImagePromptTool, sourcePrompt: string) {
  ...
  let focus = tool === 'logo-replace'
    ? '先判断本图是否应执行 Logo 替换，再识别所有真实原 Logo 所在载体、材质、曲面透视、遮挡关系、手部遮挡、礼盒内外区域、图中图以及实际适用的雕刻或印刷工艺。只有杯子、酒杯、醒酒器、酒瓶等产品本体或木盒表面已经真实存在可替换 Logo 时才 action=replace；装饰纹理、切割花纹、反光、产品轮廓、说明文字和纯背景文字均不算待替换 Logo。如果杯子和木盒等允许载体上完全没有可替换 Logo，必须 action=skip-no-logo，禁止建议新增 Logo。如果画面是人物递送礼物、收礼、送礼或仅展示人物与礼盒的生活方式图，且没有清晰可替换的杯子/木盒原 Logo，则必须 action=skip-gift-scene，整图保持原样。只保留与本图真实情况有关的条件；没有 Logo 的盒子或盒内衬不得新增 Logo。只有明确识别本图主要载体为啤酒杯时，才在独立提示词中补充啤酒杯限制：保持旧 Logo 与泡沫线/液面线的原高度关系；若杯身上部光滑、下部有竖纹或浮雕，新 Logo 必须保持原始宽高比等比缩小并完整留在上部光滑区域，不得进入纹理区。非啤酒杯图片严禁加入这段啤酒杯限制。'
  ...
  return `你是图片编辑请求的逐图提示词分配器。\n${focus}\n\n用户公共提示词：\n${cleanSourcePrompt}${constraintTask}\n\n要求：\n1. 只能筛选、整理公共提示词中适用于本图的内容，不得随意增加新任务。\n2. 删除与本图不相关、互相冲突或仅适用于其他图片的分支。\n3. 独立提示词必须可以直接交给图片编辑模型，简洁明确。${tool === 'scene-replace' ? '必须明确写成“将背景替换为……场景”，不能只写保护原图、优化光影或保持氛围。' : 'action=replace 时描述本图实际原 Logo 的载体、材质、位置、遮挡和制作工艺；只有本图明确为啤酒杯时才加入啤酒杯光滑带、纹理区与泡沫/液面高度限制。action 为 skip-no-logo 或 skip-gift-scene 时 prompt 写“保持原图不变，不执行 Logo 替换”。'}\n4. action 只能是 replace、skip-no-logo、skip-gift-scene；场景替换工具始终返回 replace。actionReason 用简体中文简述判断依据。\n5. 仅返回 JSON，不要 Markdown，固定结构：{"summary":"简短图片摘要","applicableConditions":["适用条件"],"prompt":"本图独立提示词","constraints":"只保留与本图有关的强制限制","action":"replace","actionReason":"处理决策原因"}。`;
}
