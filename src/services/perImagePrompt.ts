...
export function parsePerImagePromptResult(text: string) {
  ...
  const value = JSON.parse(match[0]) as { summary?: unknown; applicableConditions?: unknown; prompt?: unknown; constraints?: unknown };
  ...
  return { summary, applicableConditions, prompt, constraints };
}
...
export function buildPerImageAnalysisPrompt(tool: PerImagePromptTool, sourcePrompt: string) {
  ...
  if (tool === 'scene-replace') focus += ' 只要识别到木盒、礼盒或包装盒，必须在独立提示词中原样写入“木盒位置不要改变”，并锁定盒子的坐标、边界、大小、透视、朝向、前后层级以及它与杯子、酒瓶的相对位置。只要画面任意位置存在人物，包括背景人物，就必须逐人列出其位置、可见身体范围、姿态、动作、手势、遮挡与裁切，并明确写入“人物完整保留，禁止删除人物后只留下手或手臂；仅允许整体更换符合场景的穿搭，不得改变人体轮廓”。无论目标主题为何，独立提示词都必须明确写入“禁止新增任何背景酒瓶、酒类包装、酒瓶标签、贴纸、Logo、品牌名、文字或商标图案；用无文字调酒工具、普通玻璃器皿或非品牌装饰表达氛围”。';
  ...
  return `你是图片编辑请求的逐图提示词分配器。\n${focus}\n\n用户公共提示词：\n${cleanSourcePrompt}${constraintTask}\n\n要求：\n1. 只能筛选、整理公共提示词中适用于本图的内容，不得随意增加新任务。\n2. 删除与本图不相关、互相冲突或仅适用于其他图片的分支。\n3. 独立提示词必须可以直接交给图片编辑模型，简洁明确。${tool === 'scene-replace' ? '必须明确写成“将背景替换为……场景”，不能只写保护原图、优化光影或保持氛围。' : ''}\n4. 仅返回 JSON，不要 Markdown，固定结构：{"summary":"简短图片摘要","applicableConditions":["适用条件"],"prompt":"本图独立提示词","constraints":"只保留与本图有关的强制限制"}。`;
}
