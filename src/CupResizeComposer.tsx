...
import { cropCanvasDimensions, inferBorderColor, rgbToHex } from './services/cupResize';
...
function CupPlacementCanvas({
  ...
  const dragRef = useRef<{ start: Point; last: Point } | undefined>(undefined);
  ...
  const render = useCallback(() => {
    ...
    const crop = cropCanvasDimensions(cup.naturalWidth, cup.naturalHeight, cropScale);
    const sourceX = (cup.naturalWidth - crop.width) / 2;
    const sourceY = (cup.naturalHeight - crop.height) / 2;
    const renderedWidth = crop.width * cupScale;
    const renderedHeight = crop.height * cupScale;
    const left = positionRef.current.x - renderedWidth / 2;
    const top = positionRef.current.y - renderedHeight / 2;
    context.fillStyle = fillColor;
    context.fillRect(left, top, renderedWidth, renderedHeight);
    context.drawImage(cup, sourceX, sourceY, crop.width, crop.height, left, top, renderedWidth, renderedHeight);
    if (mode === 'move') {
      context.save();
  ...
  }, [brushSize, cropScale, cupScale, fillColor, mode]);
  ...
  useEffect(() => { render(); const timer = window.setTimeout(exportComposite, 120); return () => window.clearTimeout(timer); }, [render, exportComposite, cupScale, cropScale, fillColor, mode, position]);
  ...
  const pointerDown = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    dragRef.current = { start: current, last: current };
  };
  ...
  const pointerUp = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    const current = point(event);
    const context = paintRef.current?.getContext('2d');
    if (context && (mode === 'rect' || mode === 'ellipse')) {
      context.fillStyle = fillColor;
  ...
  return <div className="cup-resize-canvas"><canvas ref={canvasRef} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} /></div>;
}

export default function CupResizeComposer({ apiKey, apiBaseUrl, connectionMode, onRequestKey, onSessionStateChange, settingsHost }: {
  apiKey: string;
  ...
  const [cupScale, setCupScale] = useState(0.35);
  ...
  const select = (kind: 'scene' | 'cup', file: File) => {
    ...
    else { cupUrl && URL.revokeObjectURL(cupUrl); setCup(file); setCupUrl(URL.createObjectURL(file)); setCropScale(1); }
    return false;
  };
  const patchSettings = (patch: Partial<CupResizeSettings>) => setSettings((current) => {
    const next = { ...current, ...patch };
    return patch.imageModel ? { ...next, ...normalizeSettingsForModel(patch.imageModel, '1:1', next.imageSize) } : next;
  });
  const generate = async () => {
    if (!apiKey || (connectionMode === 'proxy' && !apiBaseUrl)) { onRequestKey(); return void message.warning('请先配置 Gemini API Key'); }
    ...
    try {
      const generated = await generateCupResizeImage({ apiKey, apiBaseUrl, model: settings.imageModel, scene, cup, compositeGuide: composite, imageSize: settings.imageSize, signal: controller.signal });
  ...
  const settingsPanel = <div className="settings-panel"><Flex justify="space-between"><Title level={4} style={{ margin: 0 }}>融合设置</Title><Tag color="blue">精确尺寸</Tag></Flex><Divider /><Form layout="vertical"><Form.Item label="图片模型"><Select value={settings.imageModel} onChange={(imageModel) => patchSettings({ imageModel })} options={Object.entries(MODEL_CAPABILITIES).map(([value, item]) => ({ value, label: item.label }))} /></Form.Item><Form.Item label="输出分辨率"><Segmented block value={settings.imageSize} onChange={(imageSize) => patchSettings({ imageSize: imageSize as CupResizeSettings['imageSize'] })} options={MODEL_CAPABILITIES[settings.imageModel].imageSizes} /></Form.Item><Alert type="info" showIcon title="位置和大小来自指导合成图" description="AI 只负责移除白底并补充自然光影、遮挡和边缘融合，不应重新设计杯子。" /></Form></div>;
  ...
  return <div className="cup-resize-page">
    ...
    {sceneUrl && cupUrl && <Card className="workflow-card" title={<Space><EditOutlined /><span>编辑指导合成图</span></Space>} extra={compositeUrl ? <Image width={44} height={44} src={compositeUrl} preview={{ mask: '预览' }} /> : null}>
      ...
      <div className="cup-resize-editor-layout"><div className="cup-resize-tools">
        ...
        <div className="cup-resize-control"><Flex justify="space-between"><Text>杯子大小</Text><Text>{Math.round(cupScale * 100)}%</Text></Flex><Slider min={0.05} max={1.5} step={0.01} value={cupScale} onChange={setCupScale} /></div>
        <div className="cup-resize-control"><Flex justify="space-between"><Text>白底画布裁切 / 放大</Text><Text>{Math.round(cropScale * 100)}%</Text></Flex><Slider min={0.35} max={2} step={0.01} value={cropScale} onChange={setCropScale} marks={{ 1: '原画布' }} /></div>
      ...
      </div><CupPlacementCanvas sceneUrl={sceneUrl} cupUrl={cupUrl} cupScale={cupScale} cropScale={cropScale} fillColor={fillColor} mode={mode} brushSize={brushSize} position={position} onPositionChange={setPosition} onDetectedColor={setFillColor} onCompositeChange={setComposite} clearToken={clearToken} /></div>
    </Card>}
