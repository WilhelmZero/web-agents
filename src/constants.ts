...
export const STORAGE_KEYS = {
  ...
  combinedReplaceSettings: "scene-studio.combined-replace-settings.v1",
  combinedReplaceScenePrompt: "scene-studio.combined-replace-scene-prompt.v1",
  combinedReplaceLogoPrompt: "scene-studio.combined-replace-logo-prompt.v1",
...
const CUP_SCALE_REQUIREMENT =
  "杯子真实尺寸为高 22.6cm、顶部杯口直径 7cm、杯肚直径 9cm。必须依据这些尺寸建立准确的真实物体尺度，杯子与桌面、餐具、礼物、装饰物、家具和人物等场景元素的比例必须符合现实世界的实际大小关系，严禁把杯子生成得过大或过小；严格保持杯子的外观、结构、颜色、材质、文字和 Logo 不变。";
...
export const BUILT_IN_LOGO_PRESETS = [
  {
    ...
    name: "玻璃杯 Logo 雕刻",
    content:
      "严格保持原始场景图的构图、画幅、视角、镜头、主体位置、玻璃杯造型、背景、光线、阴影、色彩和所有其他元素完全不变，不得移动、删除、替换或重新生成场景中的任何内容。只将提供的 Logo 自然、准确地雕刻在玻璃杯表面；Logo 顶部边缘距离杯口的实际垂直距离为 2cm，Logo 总宽度不得超过杯肚宽度的 70%。严格保持 Logo 的图形、文字、比例和细节，雕刻效果需要贴合玻璃杯曲面、透视、折射、反光与环境光，看起来像真实制作在杯体上的精细雕刻。除自然加入 Logo 外，不做任何其他修改。",
    builtIn: true,
    updatedAt: 0,
  },
  {
    ...
    content:
      "严格保持原始场景图的构图、画幅、视角、镜头、主体位置、玻璃杯造型、背景、光线、阴影、色彩和所有其他元素完全不变，不得移动、删除、替换或重新生成场景中的任何内容。只把提供的 Logo 转换为白色，并通过激光雕刻自然呈现在玻璃杯表面；Logo 顶部边缘距离杯口的实际垂直距离为 2cm，Logo 总宽度不得超过杯肚宽度的 70%。严格保持 Logo 原有图形、文字、比例和细节，不得重新设计 Logo。白色激光雕刻应具有真实细腻的磨砂蚀刻质感，正确贴合杯体曲面、透视、折射、反光和环境光。除自然加入白色 Logo 外，不做任何其他修改。",
    builtIn: true,
    updatedAt: 0,
  },
  {
    ...
    name: "木盒 Logo 激光雕刻",
    content:
      "严格保持原始场景图的构图、画幅、视角、镜头、主体位置、木盒造型、背景、光线、阴影、色彩、木纹和所有其他元素完全不变，不得移动、删除、替换或重新生成场景中的任何内容。只通过激光雕刻将提供的 Logo 自然、准确地刻在木盒表面，严格保持 Logo 的图形、文字、比例和细节，不得重新设计 Logo。雕刻应呈现真实的木材烧蚀与凹刻质感，正确贴合木盒表面的透视、木纹、材质和环境光影。除自然加入 Logo 外，不做任何其他修改。",
    builtIn: true,
    updatedAt: 0,
  },
] as const;

const BUILT_IN_LOGO_PRESETS_EN: Record<
  string,
...
export function localizeBuiltInScenePresets(
  language: "zh-CN" | "en-US",
): PromptPreset[] {
  if (language === "zh-CN") return BUILT_IN_SCENE_PRESETS;
  return BUILT_IN_SCENE_PRESETS.map((preset) => ({
    ...preset,
    ...BUILT_IN_SCENE_PRESETS_EN[preset.id],
  }));
}

export function localizeBuiltInLogoPresets(language: "zh-CN" | "en-US") {
  if (language === "zh-CN") return BUILT_IN_LOGO_PRESETS;
  return BUILT_IN_LOGO_PRESETS.map((preset) => ({
    ...preset,
    ...BUILT_IN_LOGO_PRESETS_EN[preset.id],
  }));
}
